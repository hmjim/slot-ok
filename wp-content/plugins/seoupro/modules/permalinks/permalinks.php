...
if ( class_exists( 'SU_Module' ) ) {
	...
	class SU_Permalinks extends SU_Module {
		...
		static function get_module_title() {
			return __( 'Permalink Tweaker', 'seo-ultimate-pro' );
		}
		...
		function admin_page_contents() {
			...
			if ( ! suwp::permalink_mode() ) {
				$this->print_message( 'warning', __( 'To use the Permalinks Tweaker, you must disable default (query-string) permalinks in your <a href="options-permalink.php">Permalink Settings</a>.', 'seo-ultimate-pro' ) );
			...
			$this->admin_form_group_start( __( 'Remove the URL bases of...', 'seo-ultimate-pro' ) );

			echo "<tr><td>\n";
			$this->admin_wftable_start( array(
				'taxonomy' => ' '
			,
				'before'   => __( 'Before', 'seo-ultimate-pro' )
			,
				'arrow'    => ' '
			,
				'after'    => __( 'After', 'seo-ultimate-pro' )
			) );
			...
			$this->dropdown( 'add_rule_if_conflict', array(
				'1' => __( 'term archive', 'seo-ultimate-pro' )
			,
				'0' => __( 'page', 'seo-ultimate-pro' )
			), __( 'URL Conflict Resolution', 'seo-ultimate-pro' ), __( 'If a term archive and a Page with the same slug end up having the same URL because of the term&#8217;s base being removed, the URL should be given to the %s.', 'seo-ultimate-pro' ) );
