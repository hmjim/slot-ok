...
if ( class_exists( 'SU_Module' ) ) {
	...
	class SU_Widgets extends SU_Module {
		...
		static function get_module_title() {
			return __( 'SEO Ultimate Pro Widgets', 'seo-ultimate-pro' );
		}
...
//Based on WordPress' WP_Widget_Categories & WP_Widget_Tag_Cloud
	class SU_Widget_SiloedTerms extends WP_Widget {
		...
		function __construct() {
			$widget_ops = array( 'description' => __( "On category archives, displays a list of child categories and/or posts in the category. Displays a list of top-level categories everywhere else. Powered by the SEO Ultimate Pro plugin.", 'seo-ultimate-pro' ) );
			parent::__construct( 'su_siloed_terms', __( 'Siloed Categories', 'seo-ultimate-pro' ), $widget_ops );
		}
		...
		function widget( $args, $instance ) {
			...
			$term_args = array(
				...
				'use_desc_for_title' => $use_desc_for_title ? '1' : '0',
				'echo'               => false
			);
		...
		function form( $instance ) {
			...
			?>
            <p>
                <label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title:', 'seo-ultimate-pro' ); ?></label>
                <input class="widefat" id="<?php echo $this->get_field_id( 'title' ); ?>"
                       name="<?php echo $this->get_field_name( 'title' ); ?>" type="text"
                       value="<?php echo $title; ?>"/></p>

            <p><input type="checkbox" class="checkbox" id="<?php echo $this->get_field_id( 'count' ); ?>"
                      name="<?php echo $this->get_field_name( 'count' ); ?>"<?php checked( $count ); ?> />
                <label for="<?php echo $this->get_field_id( 'count' ); ?>"><?php _e( 'Show post counts', 'seo-ultimate-pro' ); ?></label>
                <br/><input type="checkbox" class="checkbox"
                            id="<?php echo $this->get_field_id( 'use_desc_for_title' ); ?>"
                            name="<?php echo $this->get_field_name( 'use_desc_for_title' ); ?>"<?php checked( $use_desc_for_title ); ?> />
                <label for="<?php echo $this->get_field_id( 'use_desc_for_title' ); ?>"><?php _e( 'Use term descriptions in title attributes', 'seo-ultimate-pro' ); ?></label>
            </p>

            <p>
                <label for="<?php echo $this->get_field_id( 'taxonomy' ); ?>"><?php _e( 'Taxonomy:', 'seo-ultimate-pro' ) ?></label>
	...
	class SU_Widget_FooterLinks extends WP_Widget {
		...
		function __construct() {
			$widget_ops = array( 'description' => __( "Add this widget to display Deeplink Juggernaut&#8217;s Footer Links in a widget area of your choosing rather than the default wp_footer section. Powered by the SEO Ultimate Pro plugin.", 'seo-ultimate-pro' ) );
			parent::__construct( 'su_footer_autolinks', __( 'Footer Links', 'seo-ultimate-pro' ), $widget_ops );
		}
		...
		function widget( $args, $instance ) {
			...
			if ( $seo_ultimate_plus->call_module_func( 'footer-autolinks', 'autolink_footer', $unused, $args ) ) {
				$seo_ultimate_plus->set_module_var( 'footer-autolinks', 'already_outputted', true );
			}
		...
		function form( $instance ) {
			...
			?>
            <p>
                <label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title: <em>(optional)</em>', 'seo-ultimate-pro' ); ?></label>
                <input class="widefat" id="<?php echo $this->get_field_id( 'title' ); ?>"
                       name="<?php echo $this->get_field_name( 'title' ); ?>" type="text"
                       value="<?php echo $title; ?>"/></p>

            <p>
                <label><input type="radio" name="<?php echo $this->get_field_name( 'display' ); ?>"
                              value="list" <?php checked( 'list', $display ); ?>/> <?php _e( 'Display as a list', 'seo-ultimate-pro' ); ?>
                </label><br/>
                <label><input type="radio" name="<?php echo $this->get_field_name( 'display' ); ?>"
                              value="usersettings" <?php checked( 'usersettings', $display ); ?>/> <?php
					$seo_ultimate_plus->call_module_func( 'footer-autolinks-settings', 'get_admin_url', $formats_url );
					printf( __( 'Use my <a href="%s" target="_blank">footer link HTML formats</a>', 'seo-ultimate-pro' ), $formats_url );
					?></label>
            </p>
			<?php
		}
