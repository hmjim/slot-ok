...
if ( class_exists( 'SU_Module' ) ) {
	...
	class SU_Canonical extends SU_Module {
		...
		static function get_module_title() {
			return __( 'Canonicalizer', 'seo-ultimate-pro' );
		}
		...
		function init() {
			add_filter( 'su_get_setting-canonical-canonical_url_scheme', array(
				&$this,
				'filter_canonical_url_scheme'
			) );
		...
		function admin_page_contents() {
			...
			$this->checkboxes( array(
				'link_rel_canonical'      => __( 'Generate <code>&lt;link rel=&quot;canonical&quot; /&gt;</code> meta tags', 'seo-ultimate-pro' )
			,
				'http_link_rel_canonical' => __( 'Send <code>rel=&quot;canonical&quot;</code> HTTP headers', 'seo-ultimate-pro' )
			), __( 'Canonical URL Generation', 'seo-ultimate-pro' ) );
			$this->radiobuttons( 'canonical_url_scheme', array(
				''      => __( 'Use <code>http://</code> or <code>https://</code> depending on how the visitor accessed the page', 'seo-ultimate-pro' )
			,
				'http'  => __( 'Make all canonical URLs begin with <code>http://</code>', 'seo-ultimate-pro' )
			,
				'https' => __( 'Make all canonical URLs begin with <code>https://</code>', 'seo-ultimate-pro' )
			), __( 'Canonical URL Scheme', 'seo-ultimate-pro' ) );
			$this->checkboxes( array(
				'remove_nonexistent_pagination' => __( 'Redirect requests for nonexistent pagination', 'seo-ultimate-pro' )
			), __( 'Automated 301 Redirects', 'seo-ultimate-pro' ) );
			$this->child_admin_form_end();

			echo "<p class='admin_subtitle'>Click Here for<a href='admin.php?page=su-canonical-url'> Additional Canonical Settings</a>.</p>";
		}
		...
		function get_canonical_url() {
			...
			if ( get_query_var( 'm' ) ) {
				...
				file_put_contents("tag.txt","found");
				$tag = get_term_by( 'slug', get_query_var( 'tag' ), 'post_tag' );
				if ( ! empty( $tag->term_id ) ) {
					file_put_contents("tag2.txt","found");
					$link = get_tag_link( $tag->term_id );
					file_put_contents("tag3.txt",$link);
				}
