...
if ( class_exists( 'SU_Module' ) ) {
	...
	class SU_FofsLog extends SU_Module {
		...
		static function get_module_title() {
			return __( '404 Monitor Log', 'seo-ultimate-pro' );
		}

		function get_module_subtitle() {
			return __( 'Log', 'seo-ultimate-pro' );
		}
		...
		function get_admin_table_columns() {
			...
			,
				'hit-count'     => __( 'Hits', 'seo-ultimate-pro' )
			,
				'url'           => __( 'URL with 404 Error', 'seo-ultimate-pro' )
			,
				'last-hit-time' => __( 'Date of Most Recent Hit', 'seo-ultimate-pro' )
			,
				'referers'      => __( 'Referers', 'seo-ultimate-pro' )
			,
				'user-agents'   => __( 'User Agents', 'seo-ultimate-pro' )
			);
		}
		...
		function admin_page_contents() {
			...
			if ( ! $this->get_setting( 'log_enabled', true ) ) {
				$this->queue_message( 'warning', __( 'New 404 errors will not be recorded because 404 logging is disabled on the Settings tab.', 'seo-ultimate-pro' ) );
			}
			...
			if ( $this->is_action( 'delete' ) ) {
				...
				if ( isset( $the404s[ $_GET['object'] ] ) ) {
					...
					$this->queue_message( 'success', __( 'The log entry was successfully deleted.', 'seo-ultimate-pro' ) );
				} else {
					$this->queue_message( 'error', __( 'This log entry has already been deleted.', 'seo-ultimate-pro' ) );
				}
				...
				$this->queue_message( 'success', __( 'The log was successfully cleared.', 'seo-ultimate-pro' ) );
			}

			if ( ! @count( $the404s ) ) {
				$this->queue_message( 'success', __( 'No 404 errors in the log.', 'seo-ultimate-pro' ) );
			}
			...
			if ( @count( $the404s ) ) {
				...
				foreach ( $the404s as $url => $data ) {
					...
					$this->table_cells( array(
						'actions'       =>
							"<span class='su-404s-hit-open'><a href='$a_url' target='_blank'><img src='{$this->module_dir_url}hit-open.png' title='" . __( 'Open URL in new window (will not be logged)', 'seo-ultimate-pro' ) . "' /></a></span>"
							. "<span class='su-404s-hit-cache'><a href='http://www.google.com/search?q=cache%3A{$ae_url}' target='_blank'><img src='{$this->module_dir_url}hit-cache.png' title='" . __( 'Query Google for cached version of URL (opens in new window)', 'seo-ultimate-pro' ) . "' /></a></span>"
							. "<span class='su-404s-hit-delete'><a href='" . $this->get_nonce_url( 'delete', $url ) . "'><img src='{$this->module_dir_url}hit-delete.png' title='" . __( 'Remove this URL from the log', 'seo-ultimate-pro' ) . "' /></a></span>"
					,
						'hit-count'     => $data['hit_count']
					,
						'url'           => "<attr title='$a_url'>" . esc_html( sustr::truncate( $url, 100 ) ) . '</attr>'
					,
						'last-hit-time' => sprintf( __( '%s at %s', 'seo-ultimate-pro' )
							, date_i18n( get_option( 'date_format' ), $data['last_hit_time'] )
							, date_i18n( get_option( 'time_format' ), $data['last_hit_time'] )
						)
					,
						'referers'      => number_format_i18n( count( $data['referers'] ) ) . ( count( $data['referers'] ) ? " <a href='#' class='su_toggle_hide' data-toggle='su-404s-hit-$md5url-referers'><img src='{$this->module_dir_url}hit-details.png' title='" . __( 'View list of referring URLs', 'seo-ultimate-pro' ) . "' /></a>" : '' )
					,
						'user-agents'   => number_format_i18n( count( $data['user_agents'] ) ) . ( count( $data['user_agents'] ) ? " <a href='#' class='su_toggle_hide' data-toggle='su-404s-hit-$md5url-user-agents'><img src='{$this->module_dir_url}hit-details.png' title='" . __( 'View list of user agents', 'seo-ultimate-pro' ) . "' /></a>" : '' )
					) );
					...
					if ( count( $data['referers'] ) ) {
						...
						echo "\t\t\t<div><strong>" . __( 'Referring URLs', 'seo-ultimate-pro' ) . "</strong> &mdash; ";
						echo "<a href='#' class='su_toggle_up' data-toggle='su-404s-hit-$md5url-referers'>" . __( 'Hide list', 'seo-ultimate-pro' ) . "</a>";
						echo "</div>\n";
						echo "\t\t\t<ul>\n";
					...
					if ( count( $data['user_agents'] ) ) {
						...
						echo "\t\t\t<div><strong>" . __( 'User Agents', 'seo-ultimate-pro' ) . "</strong> &mdash; ";
						echo "<a href='#' class='su_toggle_up' data-toggle='su-404s-hit-$md5url-user-agents'>" . __( 'Hide list', 'seo-ultimate-pro' ) . "</a>";
						echo "</div>\n";
						echo "\t\t\t<ul>\n";
