...
if ( class_exists( 'SU_Module' ) ) {
	...
	class SU_LinkNofollow extends SU_Module {
		...
		static function get_module_title() {
			return __( 'Nofollow Manager', 'seo-ultimate-pro' );
		}
		...
		function init() {
			...
			if ( ! is_home() ) {
				unset( $filterdata['nofollow_attributes_string']['nofollow_paged_home'] );
			}
		...
		function admin_page_contents() {
			...
			$this->checkboxes( array(
				'nofollow_adjacent_post' => __( 'Adjacent post links (next post / previous post)', 'seo-ultimate-pro' )
			,
				'nofollow_category_loop' => __( 'Category links (after posts)', 'seo-ultimate-pro' )
			,
				'nofollow_category_list' => __( 'Category links (in lists)', 'seo-ultimate-pro' )
			,
				'nofollow_comment_popup' => __( 'Comment anchor links', 'seo-ultimate-pro' )
			,
				'nofollow_comment_feed'  => __( 'Comment feed links', 'seo-ultimate-pro' )
			,
				'nofollow_date_archive'  => __( 'Date-based archive links', 'seo-ultimate-pro' )
			,
				'nofollow_paged'         => __( 'Pagination navigation links (all)', 'seo-ultimate-pro' )
			,
				'nofollow_paged_home'    => __( 'Pagination navigation links (on blog home only)', 'seo-ultimate-pro' )
			,
				'nofollow_post_more'     => __( '&#8220;Read more&#8221; links', 'seo-ultimate-pro' )
			,
				'nofollow_register'      => __( 'Registration link', 'seo-ultimate-pro' )
			,
				'nofollow_login'         => __( 'Login link', 'seo-ultimate-pro' )
			,
				'nofollow_tag_loop'      => __( 'Tag links (after posts)', 'seo-ultimate-pro' )
			,
				'nofollow_tag_list'      => __( 'Tag links (in lists and clouds)', 'seo-ultimate-pro' )
			), __( 'Add the nofollow attribute to...', 'seo-ultimate-pro' ) );
		...
		function postmeta_fields( $fields, $screen ) {
			...
			if ( strcmp( $screen, 'page' ) == 0 ) {
				$fields['links'][30]['nofollow'] = $this->get_postmeta_checkbox( 'nofollow', __( 'When displaying page lists, nofollow links to this page', 'seo-ultimate-pro' ), __( 'Nofollow:', 'seo-ultimate-pro' ) );
			}
		...
		function nofollow_page_links_callback( $matches ) {
			...
			if ( preg_match( '|href=[\'"]([^\'"]+)[\'"]|i', $html, $pagematches ) ) {
				...
				$pagepath = str_replace( array(
					untrailingslashit( get_bloginfo( 'url' ) ),
					'/index.php/'
				), '', $pageurl );

				if ( preg_match( '|/?\\?page_id=([0-9]+)|i', $pagepath, $qsmatches ) ) //We're using query string URLs
				{
					$page = get_page( intval( $qsmatches[1] ) );
				} else //We're using pretty or pathinfo permalinks
				{
		...
		function add_help_tabs( $screen ) {
			...
			,
				'title'   => $this->has_enabled_parent() ? __( 'Nofollow Manager', 'seo-ultimate-pro' ) : __( 'Overview', 'seo-ultimate-pro' )
			,
				'content' => __( "
