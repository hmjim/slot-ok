...
if ( class_exists( 'SU_Module' ) ) {
	...
	class SU_SharingButtons extends SU_Module {
		...
		static function get_module_title() {
			return __( 'Sharing Facilitator', 'seo-ultimate-pro' );
		}
		...
		function get_default_settings() {
			...
			,
				'addthis_code'   => '<a class="addthis_button" href="http://addthis.com/bookmark.php?v=250"><img src="https://s7.addthis.com/static/btn/v2/lg-share-en.gif" width="125" height="16" alt="' . __( 'Bookmark and Share', 'seo-ultimate-pro' ) . '" style="border:0"/></a><script type="text/javascript" src="https://s7.addthis.com/js/250/addthis_widget.js"></script>'
			);
		}
		...
		function get_admin_page_tabs() {
			return array(
				  __('Providers', 'seo-ultimate-pro') => 'providers_tab'
			);
		}
		*/
		...
		function admin_page_contents() {
			...
			$this->radiobuttons( 'provider', array(
				'none'      => __( 'None; disable sharing buttons', 'seo-ultimate-pro' )
			,
				'sharethis' => __( 'Use the ShareThis button', 'seo-ultimate-pro' ) //: %s{sharethis_code}
			,
				'addthis'   => __( 'Use the AddThis button', 'seo-ultimate-pro' ) //: %s{addthis_code}
			), __( 'Which provider would you like to use for your sharing buttons?', 'seo-ultimate-pro' ) );
		...
		function add_sharing_buttons( $content ) {
			if ( ! is_feed() ) {
				switch ( $this->get_setting( 'provider', 'none' ) ) {
					case 'sharethis':
						$code = $this->get_setting( 'sharethis_code', '' );
						break;
					case 'addthis':
						$code = $this->get_setting( 'addthis_code', '' );
						break;
					default:
						return $content;
						break;
