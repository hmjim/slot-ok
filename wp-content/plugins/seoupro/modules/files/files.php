...
if ( class_exists( 'SU_Module' ) ) {
	...
	class SU_Files extends SU_Module {
		...
		static function get_module_title() {
			return __( 'File Editor', 'seo-ultimate-pro' );
		}
		...
		function admin_page_contents() {
			...
			if ( $is_apache ) {
				...
				if ( $is_super_admin && $exists && ! $writable ) {
					$this->queue_message( 'warning',
						__( 'A .htaccess file exists, but it&#8217;s not writable. You can edit it here once the file permissions are corrected.', 'seo-ultimate-pro' ) );
				}
			}
			...
			if ( ! strlen( get_option( 'permalink_structure' ) ) ) {
				$this->queue_message( 'error',
					__( 'WordPress won&#8217;t be able to display your robots.txt file because the default <a href="options-permalink.php" target="_blank">permalink structure</a> is in use.', 'seo-ultimate-pro' ) );
			}
			...
			$this->textarea( 'robotstxt', sprintf( __( 'robots.txt [<a href="%s" target="_blank">Open</a>]', 'seo-ultimate-pro' ), trailingslashit( get_bloginfo( 'url' ) ) . 'robots.txt' ) );

			//Show robots.txt settings checkboxes
			$this->checkboxes( array(
				'enable_custom_robotstxt'    => __( 'Enable this custom robots.txt file and disable the default file', 'seo-ultimate-pro' )
			,
				'enable_do_robotstxt_action' => __( 'Let other plugins add rules to my custom robots.txt file', 'seo-ultimate-pro' )
			), __( 'robots.txt Settings', 'seo-ultimate-pro' ) );

			$this->queue_message( 'warning',
				__( 'Please realize that incorrectly editing your robots.txt file could block search engines from your site.', 'seo-ultimate-pro' ) );

			//Of course, only bother with htaccess if we're running Apache.
			if ( $is_super_admin && $is_apache && ( $writable || ! $exists ) ) {
				$this->textarea( 'htaccess', __( '.htaccess', 'seo-ultimate-pro' ) );

				$this->queue_message( 'warning',
					__( 'Also, incorrectly editing your .htaccess file could disable your entire website. Edit with caution!', 'seo-ultimate-pro' ) );
			}
		...
		function get_htaccess() {
			if ( $this->htaccess_recovery ) {
				return $this->htaccess_recovery;
			}
		...
		function privacy_options_notice() {
			...
			if ( $pagenow == 'options-reading.php' ) { //Shows on the "Settings > Reading" page
				$this->print_message( 'info', sprintf(
					__( 'Please note that the &#8220;discourage search engines&#8221; setting won&#8217;t have any effect on your robots.txt file, since you&#8217;re using <a href="%s">a custom one</a>.', 'seo-ultimate-pro' ),
