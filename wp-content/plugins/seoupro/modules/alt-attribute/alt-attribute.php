...
if ( class_exists( 'SU_Module' ) ) {
	...
	add_filter( 'su_settings_import_array', 'alt_attribute_import_filter' );

	class SU_AltAttribute extends SU_Module {

		static function get_module_title() {
			return __( 'Alt Attribute', 'seo-ultimate-pro' );
		}
		...
		function supiat_field_input_alt( $column ) {
			if ( $column == 'supiat-alt-column' ) {
                ...
                <div class="altwrapper" id="wrapper-<?php echo $post->ID; ?>">
                    <textarea style="width: 100%; border-radius: 2px;" class="alttext"
                              id="alt-<?php echo $post->ID; ?>"><?php echo wp_strip_all_tags( get_post_meta( $post->ID, '_wp_attachment_image_alt', true ) ); ?></textarea>
                </div>
				<?php
			}
		}
		...
		function supiat_field_input_caption( $column ) {
			if ( $column == 'at_caption_text-alt-column' ) {
                ...
                <div class="altwrapper" id="wrapper-caption-<?php echo $post->ID; ?>">
                    <textarea style="width: 100%; border-radius: 2px;" class="caption"
                              id="caption-<?php echo $post->ID; ?>"><?php echo $post->post_excerpt ? $post->post_excerpt : ''; ?></textarea>
                </div>
				<?php
			}
		}
