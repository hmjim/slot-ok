...
class SU_Module {
	...
	function get_module_subtitle() {
		return isset( $this ) ? $this->get_module_title() : '';
	}
	 ...
	 */
	function get_menu_parent_hook() {
		return $this->get_menu_parent();
	}
	...
	function get_settings_key() {
		if ( isset( $this ) ) {
			...
			if ( strlen( $parent = self::get_parent_module() ) && ! self::is_independent_module() ) {
				...
				return $seo_ultimate_plus->modules[ $parent ]->get_settings_key();
			} else {
				if ( strlen( $parent = self::get_parent_module() ) && ! self::is_independent_module() ) {
					global $seo_ultimate_plus;

					return $seo_ultimate_plus->get_module_key();
				} else {
					return false;
				}
			}
	 ...
	 * 1. When the SEO Ultimate Pro plugin is activated (not necessarily for the first time)
	 ...
	 * 1. When the SEO Ultimate Pro plugin is deactivated or uninstalled.
	...
	/**
	 * Called when SEO Ultimate Pro has just been upgraded to a new version.
	 *
	 * @since 2.1
	 */
	function upgrade() {
	...
	function get_module_key() {
		...
		} else //This error will only be triggered if someone has seriously messed with the plugin architecture
		{
			die( "An SEO Ultimate Pro module did not initialize properly. Perhaps you're trying to load an SEO Ultimate Pro module independent of the plugin?" );
		}
	}
	...
	function is_module_admin_page() {
		if ( is_admin() ) {
			global $plugin_page;
			if ( strcmp( $plugin_page, $this->plugin->key_to_hook( $this->get_module_or_parent_key() ) ) == 0 ) {
				return true;
			}
		}
	...
	function children_admin_pages_form() {
		if ( count( $this->modules ) ) {
			...
			$this->print_message( 'warning', sprintf( __( 'All the modules on this page have been disabled. You can re-enable them using the <a href="%s">Module Manager</a>.', 'seo-ultimate-pro' ), $this->get_admin_url( 'modules' ) ) );
		}
	}
	...
	function get_setting( $key, $default = null, $module = null, $sneakpeak = false ) {
		...
		$msdata = (array) get_option( "seo_ultimate_plus_module_$module", array() );
	...
	function update_setting( $key, $value, $module = null, $array_key = null ) {
		...
		update_option( "seo_ultimate_plus_module_$module", $msdata );
	}
	...
	function admin_page_start( $icon = 'options-general' ) {
		...
		echo "\n<h2><img width=\"150\" height=\"50\" alt=\"logo\" src=\"" . SU_PLUGIN_ROOT . "/assets/img/Small_SEO_Ultimate_Pro_Logo.png\"> <span>" . $this->get_page_title() . "</span></h2>\n";
	}
	...
	function admin_footer() {
		printf( __( '%1$s | %2$s %3$s by %4$s', 'seo-ultimate-pro' ),
			$this->get_module_title(),
			'<a href="' . SU_PLUGIN_URI . '" target="_blank" rel="nofollow">' . __( SU_PLUGIN_NAME, 'seo-ultimate-pro' ) . '</a>',
			SU_VERSION,
			'<a href="' . SU_AUTHOR_URI . '" target="_blank" rel="nofollow">' . __( SU_AUTHOR, 'seo-ultimate-pro' ) . '</a>'
		);
	...
	function meta_edit_tab( $genus, $tab, $type, $type_label, $fields ) {
		if ( ! $this->meta_edit_table( $genus, $tab, $type, $type_label, $fields ) ) {
			$this->print_message( 'info', __( 'Your site currently doesn&#8217;t have any public items of this type.', 'seo-ultimate-pro' ) );
		}
	}
	...
	function meta_edit_table( $genus, $tab, $type, $type_label, $fields ) {
		...
		$headers = array(
			'actions' => __( 'Actions', 'seo-ultimate-pro' )
		,
			'id'      => __( 'ID', 'seo-ultimate-pro' )
		,
			'name'    => $type_label
		);
		...
		foreach ( $objects as $object ) {
			...
			$actions = array( sprintf( '<a href="%s">%s</a>', $view_url, __( 'View', 'seo-ultimate-pro' ) ) );
			if ( $edit_url ) {
				$actions[] = sprintf( '<a href="%s">%s</a>', $edit_url, __( 'Edit', 'seo-ultimate-pro' ) );
			}
			...
			foreach ( $fields as $field ) {
				...
				switch ( $field['type'] ) {
					case 'textbox':
						$counter_text = 'You&#8217;ve Entered %s Characters. Most Search Engines Use Up To 70.';
						break;
					case 'textarea':
						$counter_text = 'You&#8217;ve Entered %s Characters. Most Search Engines Use Up To 140.';
						break;
				}
				switch ( $field['name'] ) {
					case 'og_title':
	...
	function get_input_element( $type, $name, $value = null, $extra = false, $inputid = true, $args = array() ) {
		...
		if ( strlen( $inputid ) ) {
			$inputid = " id='" . su_esc_attr( $inputid ) . "'";
		}
		...
		switch ( $type ) {
			case 'textbox':
				...
				return "<input name='$name'$inputid value='$value'$placeholder type='text' class='form-control input-sm textbox regular-text' onkeyup=\"javascript:document.getElementById('" . su_esc_attr( $name ) . "_charcount').innerHTML = document.getElementById('" . su_esc_attr( $name ) . "').value.length\" /><span class=''>" . sprintf( __( $counter_text, 'seo-ultimate-pro' ), "<strong  id='" . su_esc_attr( $name ) . "_charcount'>" . strlen( $value ) . "</strong>" ) . "</span>";
				break;
			case 'textarea':
				$value = su_esc_editable_html( $value );

				return "<textarea name='$name'$inputid type='text' rows='3' cols='50' class='textarea form-control regular-text qqq'  onkeyup=\"javascript:document.getElementById('" . su_esc_attr( $name ) . "_charcount').innerHTML = document.getElementById('" . su_esc_attr( $name ) . "').value.length\">$value</textarea><span class=''>" . sprintf( __( $counter_text, 'seo-ultimate-pro' ), "<strong  id='" . su_esc_attr( $name ) . "_charcount'>" . strlen( $value ) . "</strong>" ) . "</span>";
				break;
			case 'checkbox':
			...
			case 'dropdown':
				...
				if ( is_array( $extra ) ) {
					$html .= suhtml::option_tags( $extra, $value );
				} else {
					$html .= $extra;
				}
				$html .= "</select>";
	...
	function get_admin_form_subsection( $field, $current_value, $trigger_value, $html ) {
		if ( $current_value === null ) {
			$current_value = $this->get_setting( $field );
		}
	...
	function get_admin_form_subsections( $field, $current_value, $subsections ) {
		$allhtml = '';
		if ( ! in_array( $current_value, $subsection_keys = array_keys( $subsections ) ) ) {
			$current_value = $subsection_keys[0];
		}
	 ...
	 *
	 * @since 0.1
	 * @uses SEO_Ultimate_Plus::key_to_hook()
	 * @uses get_module_key()
	...
	function admin_form_start( $header = false, $table = true, $form = true ) {
		...
		if ( $header ) {
			$this->admin_subheader( $header );
		}

		if ( $form ) {
			$hook = $this->plugin->key_to_hook( $this->get_module_or_parent_key() );
			if ( $this->is_action( 'update' ) ) {
				$this->print_message( 'success', __( 'Settings updated.', 'seo-ultimate-pro' ) );
			}
	...
	function admin_form_end( $button = null, $table = true ) {
		...
		} //This string is used in normal WP, so we don't need a textdomain
		//if ($table) echo "</table>\n";
		...
		if ( is_array( $checkboxes ) ) {
			foreach ( $checkboxes as $name => $desc ) {
				...
				if ( strpos( $desc, '%d' ) === false ) {
					...
					if ( $this->get_setting( $name ) === true ) {
						$sfdisabled = '';
					} else {
						$sfdisabled = "readonly='readonly' ";
					}
	...
	function textboxes( $textboxes, $defaults = array(), $grouptext = false, $args = array(), $textbox_args = array() ) {
		...
		foreach ( $textboxes as $id => $title ) {
			...
			$id           = su_esc_attr( $id );
			$resetmessage = su_esc_attr( __( 'Are you sure you want to replace the textbox contents with this default value?', 'seo-ultimate-pro' ) );
			...
			if ( $grouptext ) {
				...
				echo "<div class='form-group'$indentattrs$hidden>\n<label class='col-sm-3 col-md-3 control-label' for='$id'><span class='su-field-label-text'>$title</span></label>\n<div class='col-sm-4 col-md-4'>\n";
			} elseif ( $in_table ) {
				echo "<div class='form-group'$indentattrs$hidden>\n<label class='col-sm-3 col-md-3 control-label' for='$id'><span class='su-field-label-text'>$title</span></label>\n<div class='col-sm-4 col-md-4'>\n";
			}
			...
			if ( $help_text ) {
				echo "<div class='col-sm-3 col-md-3 help-text'>$help_text</div>\n<div class='col-sm-2 col-md-2'>" . sprintf( __( $counter_text, 'seo-ultimate-pro' ), "<strong  id='{$id}_charcount'>" . strlen( $value ) . "</strong>" ) . "</div>\n</div>\n";
			} else {
	...
	function textareas( $textareas, $rows = 5, $cols = 30, $args = array() ) {
		...
		foreach ( $textareas as $id => $title ) {
			...
			echo "<div class='form-group'>\n";
			if ( $title ) {
				echo "<label class='col-sm-3 col-md-3 control-label' for='$id'>$title</label>\n";
			}
	...
	function jlsuggest_boxes( $jls_boxes ) {
		...
		foreach ( $jls_boxes as $jls_box ) {
			...
			echo "<div class='form-group'>\n";
			if ( $title ) {
				echo "<label class='col-sm-4 col-md-4 control-label' for='$id'>$title</label>\n";
			}
			echo "<div class='col-sm-4 col-md-4'>";
...
$title="";
	...
	function get_nonce_handle( $action, $id = false ) {
		...
		if ( ! $key || ! $this->plugin->module_exists( $key ) ) {
			$key = $this->get_module_key();
		}
	...
	function get_nonce_url( $action, $object = false ) {
		...
		if ( $object ) {
			$objectqs = '&object=' . urlencode( $object );
		} else {
			$objectqs = '';
		}
	...
	function get_postmeta_textboxes( $textboxes, $textbox_args = array(), $grouptext = false ) {
		...
		foreach ( $textboxes as $id => $title ) {
			...
			register_setting( 'seo-ultimate-pro', $id );
	...
	function get_postmeta_checkboxes( $checkboxes, $grouptext ) {
		...
		if ( is_array( $checkboxes ) ) {
			foreach ( $checkboxes as $name => $desc ) {
				...
				register_setting( 'seo-ultimate-pro', $name );
	 ...
	 * @uses SEO_Ultimate_Plus::queue_js()
	 * @uses SEO_Ultimate_Plus::queue_css()
	 */
	...
	/**
	 * Outputs the SEO Ultimate Pro XMLNS used by JLSuggest.
	 *
	 * @since 6.0
	 */
	...
	function get_jlsuggest_box( $name, $value, $params = '', $placeholder = '' ) {
			...
			case 'posttype':
				...
				if ( $selected_post ) {
					...
					if ( $selected_post_type ) {
						$text_dest = sprintf( __( 'A Deleted %s', 'seo-ultimate-pro' ), $selected_post_type->labels->singular_name );
					} else {
						$text_dest = __( 'A Deleted Post', 'seo-ultimate-pro' );
					}
			...
			case 'taxonomy':
				if ( $selected_taxonomy = get_taxonomy( $to_type ) ) {
					if ( $selected_term = get_term( $to_id, $selected_taxonomy->name ) ) {
						...
						$text_dest = sprintf( __( 'A Deleted %s', 'seo-ultimate-pro' ), $selected_taxonomy->labels->singular_name );
						$text_dest = '<span class="type">' . $text_dest . '</span>';
						$disabled  = true;
					}
				} else {
					$text_dest = __( 'A Deleted Term', 'seo-ultimate-pro' );
					$text_dest = '<span class="type">' . $text_dest . '</span>';
					$disabled  = true;
				}
				break;
			case 'home':
				$text_dest = __( 'Blog Homepage', 'seo-ultimate-pro' );
				break;
			case 'author':
				if ( is_user_member_of_blog( $to_id ) ) {
					$selected_author = get_userdata( $to_id );
					$text_dest       = $selected_author->user_login . '<span class="type">&nbsp;&mdash;&nbsp;' . __( 'Author', 'seo-ultimate-pro' ) . '</span>';
				} else {
					$text_dest = __( 'A Deleted User', 'seo-ultimate-pro' );
					$text_dest = '<span class="type">' . $text_dest . '</span>';
					$disabled  = true;
				}
				break;
			case 'internal-link-alias':
				...
				if ( isset( $aliases[ $to_id ]['to'] ) ) {
					...
					if ( $this->plugin->module_exists( 'internal-link-aliases' ) ) {
						$text_dest .= __( 'Link Mask', 'seo-ultimate-pro' );
					} else {
						$text_dest .= __( 'Link Mask (Disabled)', 'seo-ultimate-pro' );
						$disabled  = true;
					}
					$text_dest .= '</span>';
				} else {
					$text_dest = __( 'A Deleted Link Mask', 'seo-ultimate-pro' );
					$text_dest = '<span class="type">' . $text_dest . '</span>';
					$disabled  = true;
				}
		...
		$html .= ' title="' . __( 'Type a URL or start typing the name of an item on your site', 'seo-ultimate-pro' ) . '"';
		...
		$html     .= $text_dest;
		$html     .= '</div>';
		$html     .= '<div><a href="#" onclick="javascript:return false;" class="jls_text_dest_close" title="' . __( 'Remove this location from this textbox', 'seo-ultimate-pro' ) . '">' . __( 'X', 'seo-ultimate-pro' ) . '</a></div>';
		$html     .= '</div>';
	...
	function promo_sdf_banners() {
		...
		if ( $this->should_show_sdf_theme_promo() ) {
            ...
            <div id="sds_promo_blog_post" class="hide">
				<?php $this->promo_sdf_banners_rss_output( 'http://feeds.seodesignsolutions.com/SeoDesignSolutionsBlog', array(
					'show_summary' => 1,
					'show_date'    => 0,
					'items'        => 1
				) ); ?>
