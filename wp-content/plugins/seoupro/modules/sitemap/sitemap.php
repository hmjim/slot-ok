...
if ( class_exists( 'SU_Module' ) ) {
	...
	if ( !is_multisite() && @$psdata['modules'][ 'sitemap' ] == 10) {


		class Su_Sitemap extends SU_Module {

			var $namespaces_declared = false;
			var $jlsuggest_box_post_id = false;

			static function get_module_title() {
				return __( 'Sitemap Creator', 'seo-ultimate-pro' );
			}
			...
			public static function pluginSettingsLink( $links ) {
				return array_merge( $links, array( sprintf( '<a href="%s">%s</a>', esc_url( admin_url( 'admin.php?page=su-sitemap' ) ), __( 'Settings' ) ) ) );
			}
			...
			public static function sitemapAdminInit() {
				foreach (
					array(
						'simple_wp_other_urls',
						'simple_wp_block_urls',
						'simple_wp_attr_link',
						'simple_wp_disp_categories',
						'simple_wp_disp_tags',
						'simple_wp_disp_authors',
						'simple_wp_disp_sitemap_order',
						'simple_wp_sitemap_version',
						'simple_wp_last_updated'
					) as $setting
				) {
			...
			public static function generateSitemapContent() {
				global $wp_query;
				if ( isset( $wp_query->query_vars['thesimplewpsitemap'] ) && in_array( ( $q = $wp_query->query_vars['thesimplewpsitemap'] ), array(
						'xml',
						'html'
					) ) ) {
	...
	*/
                ...
                <div class="wrap">
                    ...
                    <form method="post" action="<?php echo $options->getSubmitUrl(); ?>"
                          id="simple-wp-sitemap-form">
                        ...
                        <input type="hidden" id="simple_wp_active-page" name="simple_wp_active-page"
                               value="<?php echo $options->getPage(); ?>">

                        <table id="sitemap-table-show" class="widefat form-table table-hidden"
                               data-id="sitemap-normal">
                            <tr>
                                <td><strong>Add pages</strong></td>
                            </tr>
                            <tr>
                                <td>Add pages to the sitemaps in addition to your normal wordpress ones. Just paste
                                    "full"
                                    urls in the textarea like: <strong>http://www.example.com/a-page/</strong>. Each
                                    link on
                                    a new row <em>(this will affect both your xml and html sitemap)</em>.
                                </td>
                            </tr>
                            <tr>
                                <td><textarea rows="7" name="simple_wp_other_urls"
                                              placeholder="http://www.example.com/a-page/" class="large-text code"
                                              id="swsp-add-pages-textarea"><?php echo $options->getOptions( 'simple_wp_other_urls' ); ?></textarea>
                                </td>
                            </tr>
                            ...
                            <tr>
                                <td>Add pages you want to block from showing up in the sitemaps. Same as above, just
                                    paste
                                    every link on a new row. <em>(Hint: copy paste links from one of the sitemaps to
                                        get
                                        correct urls)</em>.
                                </td>
                            </tr>
                            <tr>
                                <td><textarea rows="7" name="simple_wp_block_urls"
                                              placeholder="http://www.example.com/block-this-page/"
                            ...
                            <tr>
                                <td>Check if you want to include categories, tags and/or author pages in the
                                    sitemaps.
                                </td>
                            </tr>
                            <tr>
                        ...
                        <table class="widefat form-table table-hidden" data-id="sitemap-advanced">
                            <tr>
                                <td><strong>Display order &amp; titles</strong></td>
                            </tr>
                            <tr>
                                <td>
                                    If you want to change the display order in your sitemaps, click the arrows to
                                    move
                                    sections up or down. They will be displayed as ordered below <em>(highest up is
                                        displayed first and lowest down last)</em>.<br><br>
                            ...
                            <tr>
                                <td><strong>Last updated text:</strong> <input type="text"
                                                                               name="simple_wp_last_updated"
                                                                               placeholder="Last updated"
                                                                               value="<?php echo $options->getOptions( 'simple_wp_last_updated' ); ?>"
                                                                               id="simple_wp_last_updated"></td>
                            </tr>
                            <tr>
                                <td><input type="button" id="sitemap-defaults" class="button-secondary"
                                           title="Restore the default display order" value="Restore defaults"></td>
                            </tr>
                        </table>

                        <p class="submit"><input type="submit" class="button-primary" value="Save Changes"></p>
                        <p><em>(If you have a caching plugin, you might have to clear cache before changes will be
                                shown in
                                the sitemaps)</em></p>
                    </form>
