...
if ( class_exists( 'SU_Module' ) ) {
	...
	function su_user_code_plus_import_filter( $all_settings ) {
	...
	add_filter( 'su_settings_import_array', 'su_user_code_plus_import_filter' );

	class SU_UserCodePlus extends SU_Module {

		static function get_module_title() {
			return __( 'Code Inserter +', 'seo-ultimate-pro' );
		}
		...
		function init() {
			...
			foreach ( $hooks as $hook ) {
				add_filter( $hook, array( &$this, "{$hook}_code" ), 'su_head' == $hook ? 11 : 10 );
			}
		}
		...
		function su_head_code() {
			...
			echo $this->plugin->mark_code( $code, __( 'SEO Ultimate+Code Inserter module', 'seo-ultimate-pro' ), 'wp_head' );
		}
		...
		function get_arr_settings() {
			return array(
				...
				'label'             => __( 'Title Tag', 'seo-ultimate-pro' ),
				'dop'               => array(
					array(
						'type'              => 'textarea',
						'name'              => 'header',
						'term_settings_key' => 'taxonomy_header',
						'label'             => __( 'Header', 'seo-ultimate-pro' )
					),
					array(
						'type'              => 'textarea',
						'name'              => 'before',
						'term_settings_key' => 'taxonomy_before',
						'label'             => __( 'Above Content', 'seo-ultimate-pro' )
					),
					array(
						'type'              => 'textarea',
						'name'              => 'after',
						'term_settings_key' => 'taxonomy_after',
						'label'             => __( 'Below Content', 'seo-ultimate-pro' )
					),
					array(
						'type'              => 'textarea',
						'name'              => 'footer',
						'term_settings_key' => 'taxonomy_footer',
						'label'             => __( 'Footer', 'seo-ultimate-pro' )
					)
				)
			);
		...
		function get_admin_page_tabs() {
			return array_merge(
			/*array(
				array('title' => __('Default Formats', 'seo-ultimate-pro'), 'id' => 'su-default-formats', 'callback' => 'formats_tab')
			, array('title' => __('Settings', 'seo-ultimate-pro'), 'id' => 'su-settings', 'callback' => 'settings_tab')
			)
			,*/
							...
							if ( isset( $field['dop'] ) && is_array( $field['dop'] ) ) {
								foreach ( $field['dop'] as $item ) {
									if ( preg_match( "/{$genus}_([0-9]+)_{$item['name']}/", $key, $matches ) ) {
										...
										$arr[ $id ][] = array(
											'type'              => $item['type'],
											'term_settings_key' => $item['term_settings_key'],
											'name'              => "_su_{$item['name']}",
											'label'             => $value
										);
									}
					...
					if ( is_array( $field['dop'] ) ) {
						foreach ( $field['dop'] as $item ) {
							...
							if ( $is_update && $item['type'] == 'checkbox' && $value == '1' && ! isset( $_POST[ $inputid ] ) )
								switch ( $genus ) {
									case 'post':
										delete_post_meta( $id, "_su_{$item['name']}" );
										$value = 0;
										break;
									case 'term':
										$this->update_setting( $item['term_settings_key'], false, null, $id );
										break;
								}
		...
		function settings_tab() {
			...
			$this->checkbox( 'terms_ucwords', __( 'Convert lowercase category/tag names to title case when used in title tags.', 'seo-ultimate-pro' ), __( 'Title Tag Variables', 'seo-ultimate-pro' ) );
			$this->radiobuttons( 'rewrite_method', array(
				'ob'     => __( 'Use output buffering &mdash; no configuration required, but slower (default)', 'seo-ultimate-pro' )
			,
				'filter' => __( 'Use filtering &mdash; faster, but configuration required (see the &#8220;Settings Tab&#8221 section of the &#8220;Help&#8221; dropdown for details)', 'seo-ultimate-pro' )
			), __( 'Rewrite Method', 'seo-ultimate-pro' ) );
		...
		function get_default_settings() {
			...
			return array(
				'title_home'     => __( '{blog}', 'seo-ultimate-pro' )
			,
				'title_single'   => __( '{post} | {blog}', 'seo-ultimate-pro' )
			,
				'title_page'     => __( '{page} | {blog}', 'seo-ultimate-pro' )
			,
				'title_category' => __( '{category} | {blog}', 'seo-ultimate-pro' )
			,
				'title_tag'      => __( '{tag} | {blog}', 'seo-ultimate-pro' )
			,
				'title_day'      => __( 'Archives for {month} {day}, {year} | {blog}', 'seo-ultimate-pro' )
			,
				'title_month'    => __( 'Archives for {month} {year} | {blog}', 'seo-ultimate-pro' )
			,
				'title_year'     => __( 'Archives for {year} | {blog}', 'seo-ultimate-pro' )
			,
				'title_author'   => __( 'Posts by {author} | {blog}', 'seo-ultimate-pro' )
			,
				'title_search'   => __( 'Search Results for {query} | {blog}', 'seo-ultimate-pro' )
			,
				'title_404'      => __( '404 Not Found | {blog}', 'seo-ultimate-pro' )
			,
				'title_paged'    => __( '{title} - Page {num}', 'seo-ultimate-pro' )
			,
				'terms_ucwords'  => true
			,
				'rewrite_method' => 'ob'
		...
		function get_supported_settings() {
			return array(
				'title_home'     => __( 'Blog Homepage Title', 'seo-ultimate-pro' )
			,
				'title_single'   => __( 'Post Title Format', 'seo-ultimate-pro' )
			,
				'title_page'     => __( 'Page Title Format', 'seo-ultimate-pro' )
			,
				'title_category' => __( 'Category Title Format', 'seo-ultimate-pro' )
			,
				'title_tag'      => __( 'Tag Title Format', 'seo-ultimate-pro' )
			,
				'title_day'      => __( 'Day Archive Title Format', 'seo-ultimate-pro' )
			,
				'title_month'    => __( 'Month Archive Title Format', 'seo-ultimate-pro' )
			,
				'title_year'     => __( 'Year Archive Title Format', 'seo-ultimate-pro' )
			,
				'title_author'   => __( 'Author Archive Title Format', 'seo-ultimate-pro' )
			,
				'title_search'   => __( 'Search Title Format', 'seo-ultimate-pro' )
			,
				'title_404'      => __( '404 Title Format', 'seo-ultimate-pro' )
			,
				'title_paged'    => __( 'Pagination Title Format', 'seo-ultimate-pro' )
			);
		}
		...
		function postmeta_help( $help ) {
			$help[] = __( '<strong>Title Tag</strong> &mdash; The exact contents of the &lt;title&gt; tag. The title appears in visitors&#8217; title bars and in search engine result titles. If this box is left blank, then the <a href="admin.php?page=su-titles" target="_blank">default post/page titles</a> are used.', 'seo-ultimate-pro' );
...
", 'seo-ultimate-pro' )
			...
			,
				'title'   => __( 'Troubleshooting', 'seo-ultimate-pro' )
			,
				'content' => __( "
<ul>
