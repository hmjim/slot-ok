...
if ( class_exists( 'SU_Module' ) ) {
	...
	class SU_CanonicalURL extends SU_Module {

		static function get_module_title() {
			return __( 'Global Canonical Manager', 'seo-ultimate-pro' );
		}
		...
		function init() {
			...
			$this->su_canonical = new SU_Canonical();
			//If the canonical tags are enabled, then...
			if ( $this->su_canonical->get_setting( 'link_rel_canonical' ) ) {
			...
			if ( $this->su_canonical->get_setting( 'http_link_rel_canonical' ) ) {
		...
		function get_admin_page_tabs() {
			return array_merge(
				$this->get_meta_edit_tabs( array(
					...
					'label'             => __( 'Canonical URL', 'seo-ultimate-pro' ),
				) )
			);
		}
		...
		function link_rel_canonical_tag() {
			...
			if ( $scheme = $this->su_canonical->get_setting( 'canonical_url_scheme', 'http' ) ) {
		 ...
		 * which in turn was heavily based on the {@link Permalink Redirect} plugin.
		 */
...
$link = "";
		...
		function postmeta_fields( $fields, $screen ) {
			$id                                          = "_su_canonical_url_tag";
			$value                                       = su_esc_attr( $this->get_postmeta( 'canonical_url_tag' ) );
			$fields['advanced'][30]['canonical_url_tag'] =
				"<div class='form-group su textbox'>\n<label class='col-sm-4 col-md-4 control-label' for='$id'>" . __( 'Canonical URL:', 'seo-ultimate-pro' ) . "</label>\n<div class='col-sm-4 col-md-4'><input name='$id' id='$id' type='text' value='$value' class='form-control input-sm regular-text' tabindex='2'"
				. " />"
				. "</div>\n<div class='col-sm-4 col-md-4 help-text'>" . sprintf( __( 'The Canonical URL that this page should point to. Leave field empty to default to existing permalink.', 'seo-ultimate-pro' ) )
				. "</div>\n</div>\n";
