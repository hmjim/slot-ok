...
if ( class_exists( 'SU_Module' ) ) {
	...
	class SU_Linkbox extends SU_Module {
		...
		static function get_module_title() {
			return __( 'Linkbox Inserter', 'seo-ultimate-pro' );
		}
		...
		function get_default_settings() {
			...
			return array(
				'html' => '<div class="su-linkbox" id="post-{id}-linkbox"><div class="su-linkbox-label">' .
				          __( 'Link to this post!', 'seo-ultimate-pro' ) .
		...
		function template_init() {
			...
			if ( $this->should_linkbox() ) //Add the linkbox to post/page content
			{
				add_filter( 'the_content', array( &$this, 'linkbox_filter' ) );
			}

			if ( $this->get_setting( 'action_hook' ) ) //Enable the action hook
			{
		...
		function admin_page_contents() {
			...
			$this->checkboxes( array(
				'filter_posts' => __( 'At the end of posts', 'seo-ultimate-pro' )
			,
				'filter_pages' => __( 'At the end of pages', 'seo-ultimate-pro' )
			,
				'action_hook'  => __( 'When called by the su_linkbox hook', 'seo-ultimate-pro' )
			), __( 'Display linkboxes...', 'seo-ultimate-pro' ) );
			$this->textarea( 'html', __( 'Linkbox HTML', 'seo-ultimate-pro' ) );
...
", 'seo-ultimate-pro' );
			...
			if ( $this->has_enabled_parent() ) {
				$screen->add_help_tab( array(
					'id'      => 'su-linkbox-help'
				,
					'title'   => __( 'Linkbox Inserter', 'seo-ultimate-pro' )
				,
					'content' =>
						'<h3>' . __( 'Overview', 'seo-ultimate-pro' ) . '</h3>' . $overview .
						'<h3>' . __( 'Settings Help', 'seo-ultimate-pro' ) . '</h3>' . $settings
				) );
			} else {
				...
				,
					'title'   => __( 'Settings Help', 'seo-ultimate-pro' )
				,
					'content' => $settings
				) );
			}
