...
if ( class_exists( 'SU_Module' ) ) {
	...
	class SU_OpenGraph extends SU_Module {
		...
		static function get_module_title() {
			return __( 'Open Graph+', 'seo-ultimate-pro' );
		}
		...
		function head_tag_output() {
            ...
            */
			...
			switch ( $doctype ) {
				case 'xhtml':
					$output_formats = array( '<meta%3$s name="%1$s" content="%2$s" />' => array_merge( $tags, $gplus_tags, $pinterest_meta_tag, $twitter_tags, $gplus_publisher ) );
					break;
				case 'html5':
					$output_formats = array( '<meta%3$s property="%1$s" content="%2$s">' => array_merge( $tags, $gplus_tags, $pinterest_meta_tag, $twitter_tags, $gplus_publisher ) );
					break;
				default:
					$output_formats = array(
		...
		function get_admin_page_tabs() {
			...
			$postmeta_edit_tabs = $this->get_postmeta_edit_tabs( array(
				array(
					'type'    => 'dropdown'
				,
					'options' => array_merge( array( '' => __( 'Use default', 'seo-ultimate-pro' ) ), $this->get_type_options() )
				,
					'name'    => 'og_type'
				,
					'label'   => __( 'Open Graph Type', 'seo-ultimate-pro' )
				)
			,
				array(
					'type'  => 'textbox'
				,
					'name'  => 'og_title'
				,
					'label' => __( 'Open Graph Title', 'seo-ultimate-pro' )
				)
			,
				array(
					'type'  => 'textarea'
				,
					'name'  => 'og_description'
				,
					'label' => __( 'Open Graph Description', 'seo-ultimate-pro' )
				)
			,
				array(
					'type'    => 'jlsuggest'
				,
					'name'    => 'og_image'
				,
					'label'   => __( 'Open Graph Image', 'seo-ultimate-pro' )
				,
					'options' => array(
			...
			return array_merge(
				array(
					array(
						'title'    => __( '<div class="fb-icons">Facebook</div>', 'seo-ultimate-pro' ),
						'id'       => 'su-facebook-settings',
						'callback' => 'facebook_tab'
					)
				,
					array(
						'title'    => __( '<div class="twitter-icons">Twitter</div>', 'seo-ultimate-pro' ),
						'id'       => 'su-twitter-settings',
						'callback' => 'twitter_tab'
					)
				/*
                ,
					array(
						'title'    => __( '<div class="pinterest-icons">Pinterest</div>', 'seo-ultimate-pro' ),
						'id'       => 'su-pinterest-settings',
						'callback' => 'pinterest_tab'
					)
				*/
				)
				, $postmeta_edit_tabs
		...
		function facebook_tab() {
			...
			$this->textbox( 'og_title', __( 'Open Graph Title:', 'seo-ultimate-pro' ), false, false, array(
				'help_text'    => 'Enter Default Open Graph Title.',
				'counter_text' => 'You&#8217;ve Entered %s Characters. It’s Best to Stay Between 60 and 90 Characters.'
			) );
			$this->textarea( 'og_description', __( 'Open Graph Description:', 'seo-ultimate-pro' ), false, false, array(
				'help_text'    => 'Enter Default Open Graph Description.',
				'counter_text' => 'You&#8217;ve Entered %s Characters. Keep Description to 200 Characters Max.'
			) );
			$this->medialib_box( 'og_image', __( '<a class="tooltips">Open Graph Image:<span>Use image at least 1200 X 630 for best display on high resolution devices. Minimum recommended image is 600 x 315 pixels.</span></a>', 'seo-ultimate-pro' ), 'types=posttype_attachment&post_mime_type=image/*' );
			$posttypes = get_post_types( array( 'public' => true ), 'objects' );
			echo "<br><p class='admin_subtitle'>Enter Default Open Graph Type</p>";
			$this->admin_wftable_start( array(
				'posttype' => __( 'Post Type', 'seo-ultimate-pro' )
			,
				'og'       => __( 'Open Graph Type', 'seo-ultimate-pro' )
			) );
			...
			$this->textbox( 'fb_app_id', __( 'Facebook App ID:', 'seo-ultimate-pro' ), false, false, array( 'help_text' => 'Enter Facebook App ID associated with website.' ) );
			$this->textbox( 'fb_admins_url', __( 'Facebook Admins:', 'seo-ultimate-pro' ), false, false, array( 'help_text' => 'Enter Facebook admin name/number (located under Facebook General Account Settings > Username). If more than one admin exists, enter all separated by commas.' ) );
			echo "<br><p class='admin_subtitle'>Select Global Markup Validation Type</p>";
			$this->radiobuttons( 'doctype', array(
				''      => __( 'Use the non-validating code prescribed by Open Graph and Twitter', 'seo-ultimate-pro' )
			,
				'xhtml' => __( 'Alter the code to validate as XHTML', 'seo-ultimate-pro' )
			,
				'html5' => __( 'Alter the code to validate as HTML5', 'seo-ultimate-pro' )
			), __( 'HTML Validation', 'seo-ultimate-pro' ) );
			$this->checkbox( 'enable_og_article_author', __( 'Include author data for posts', 'seo-ultimate-pro' ), __( 'Open Graph Data', 'seo-ultimate-pro' ) );
		...
		function twitter_tab() {
			...
			$this->admin_wftable_start( array(
				'posttype' => __( 'Post Type', 'seo-ultimate-pro' )
			,
				'twitter'  => __( 'Twitter Card Type', 'seo-ultimate-pro' )
			) );
			foreach ( $posttypes as $posttype ) {
				echo "<tr valign='middle'>\n";
				echo "\t<th class='su-opengraph-posttype' scope='row'>" . esc_html( $posttype->labels->name ) . "</th>\n";
			...
			$this->textbox( 'twitter_site_handle', __( 'Twitter Site:', 'seo-ultimate-pro' ), false, false, array(
				'help_text' => 'The Twitter @username the card should be attributed to.',
				'callout'   => ''
			) );
			$this->textbox( 'twitter_site_id_handle', __( 'Twitter Site ID:', 'seo-ultimate-pro' ), false, false, array( 'help_text' => 'Same as twitter:site, but the user&#39;s Twitter ID. *Either twitter:site or twitter:site:id is required.' ) );
			$this->textbox( 'twitter_creator_handle', __( 'Twitter Creator:', 'seo-ultimate-pro' ), false, false, array( 'help_text' => '@username of content creator' ) );
			$this->textbox( 'twitter_creator_id_handle', __( 'Twitter Creator ID:', 'seo-ultimate-pro' ), false, false, array( 'help_text' => 'Twitter user ID of content creator. Used with summary, summary_large_image, photo, gallery, product cards' ) );
			$this->textbox( 'twitter_title_handle', __( 'Twitter Title:', 'seo-ultimate-pro' ), false, false, array( 'help_text' => 'Title should be concise and will be truncated at 70 characters.' ) );
			$this->textarea( 'twitter_description_handle', __( 'Twitter Description:', 'seo-ultimate-pro' ), false, false, array( 'help_text' => 'Description of content (maximum 200 characters)' ) );
			$this->medialib_box( 'twitter_image_handle', __( '<a class="tooltips">Twitter Image:<span>Use button to upload image. It must be less than 1MB in size.</span></a>', 'seo-ultimate-pro' ), 'types=posttype_attachment&post_mime_type=image/*', false, false, array( 'help_text' => 'Use Button To Upload Image. It Must Be Less Than 1MB In Size.' ) );
			echo "<br><p class='admin_subtitle'>Player Card: <span style='font-weight:normal'>A Card to provide video/audio/media.</span></p>";
			$this->textbox( 'twitter_player_handle', __( 'Twitter Player:', 'seo-ultimate-pro' ), false, false, array( 'help_text' => 'HTTPS URL of player iframe' ) );
			$this->textbox( 'twitter_player_width_handle', __( 'Player Width:', 'seo-ultimate-pro' ), false, false, array( 'help_text' => 'Width of iframe in pixels' ) );
			$this->textbox( 'twitter_player_height_handle', __( 'Player Height:', 'seo-ultimate-pro' ), false, false, array( 'help_text' => 'Height of iframe in pixels' ) );
			$this->textbox( 'twitter_player_stream_handle', __( 'Player Stream:', 'seo-ultimate-pro' ), false, false, array( 'help_text' => 'URL to raw video or audio stream' ) );
			$this->textbox( 'twitter_player_stream_content_type_handle', __( 'Player Stream Content Type:', 'seo-ultimate-pro' ), false, false, array( 'help_text' => 'The MIME type/subtype combination that describes the content contained in twitter:player:stream. Takes the form specified in RFC 6381. Currently supported content_type values are those defined in RFC 4337 (MIME Type Registration for MP4)' ) );
			...
			$this->textbox( 'twitter_app_name_iphone_handle', __( 'Twitter App Name iPhone:', 'seo-ultimate-pro' ), false, false, array( 'help_text' => 'Name of your iPhone app' ) );
			$this->textbox( 'twitter_app_id_iphone_handle', __( 'Twitter App ID iPhone:', 'seo-ultimate-pro' ), false, false, array( 'help_text' => 'Your app ID in the iTunes App Store (Note: NOT your bundle ID)' ) );
			$this->textbox( 'twitter_app_url_iphone_handle', __( 'Twitter App URL iPhone:', 'seo-ultimate-pro' ), false, false, array( 'help_text' => 'Your app&#8217;s custom URL scheme (you must include "://" after your scheme name)' ) );
			$this->textbox( 'twitter_app_name_ipad_handle', __( 'Twitter App Name iPad:', 'seo-ultimate-pro' ), false, false, array( 'help_text' => 'Name of your iPad optimized app' ) );
			$this->textbox( 'twitter_app_id_ipad_handle', __( 'Twitter App ID iPad:', 'seo-ultimate-pro' ), false, false, array( 'help_text' => 'Your app ID in the iTunes App Store' ) );
			$this->textbox( 'twitter_app_url_ipad_handle', __( 'Twitter App URL iPad:', 'seo-ultimate-pro' ), false, false, array( 'help_text' => 'Your app&#8217;s custom URL scheme' ) );
			$this->textbox( 'twitter_app_name_googleplay_handle', __( 'Twitter App Name Googleplay:', 'seo-ultimate-pro' ), false, false, array( 'help_text' => 'Name of your Android app' ) );
			$this->textbox( 'twitter_app_id_googleplay_handle', __( 'Twitter App ID Googleplay:', 'seo-ultimate-pro' ), false, false, array( 'help_text' => 'Your app ID in the Google Play Store' ) );
			$this->textbox( 'twitter_app_url_googleplay_handle', __( 'Twitter App URL Googleplay:', 'seo-ultimate-pro' ), false, false, array( 'help_text' => 'Your app#8217;s custom URL scheme' ) );
			$this->admin_form_table_end();
		}
		...
		function googleplus_tab() {
			...
			$this->textbox( 'gplus_title', __( 'Google Plus Title:', 'seo-ultimate-pro' ), false, false, array(
				'help_text'    => 'Enter Default Open Graph Title.',
				'counter_text' => 'You&#8217;ve Entered %s Characters. Title Should Not Exceed 60 Characters.'
			) );
			$this->textarea( 'gplus_description', __( 'Google Plus Description:', 'seo-ultimate-pro' ), false, false, array(
				'help_text'    => 'Enter Default Open Graph Description.',
				'counter_text' => 'You&#8217;ve Entered %s Characters. Description Should Not Exceed 156 Characters.'
			) );
			$this->medialib_box( 'gplus_image_src', __( '<a class="tooltips">Google Plus Image:<span>The height must be at least 120px, and if the width is less than 100px, then the aspect ratio must be no greater than 3.0.</span></a>', 'seo-ultimate-pro' ), 'types=posttype_attachment&post_mime_type=image/*' );
			echo "<br><p class='admin_subtitle'>Google Authorship Markup Settings</p>";
			$this->textbox( 'gplus_page_url', __( 'Google Plus Publisher:', 'seo-ultimate-pro' ), false, false, array( 'help_text' => 'Enter your Google Plus page URL here if you have set up a "Google+ Page" for your organization or product, and the plugin will put a rel="publisher" link to the specified Google+ page on your home page.' ) );
			$this->admin_form_table_end();
		}
		...
		function pinterest_tab() {
			...
			$this->textbox( 'pinterest_verification', __( 'Pinterest Verification:', 'seo-ultimate-pro' ) );
			$this->admin_form_table_end();
		}

		function postmeta_fields( $fields, $screen ) {
			$id                                 = "_su_og_title";
			$value                              = su_esc_attr( $this->get_postmeta( 'og_title' ) );
			$fields['facebook'][50]['og_title'] =
				"<div class='form-group su textbox'>\n<label class='col-sm-4 col-md-4 control-label' for='$id'>" . __( 'Open Graph Title: <a class="tooltips"> <img class="og-info-icon" src="' . plugins_url( 'opengraph/og-info-icon.png', dirname( __FILE__ ) ) . '" > <span>Enter preferred title, description and image for Facebook / Open Graph. Additional social properties such as also use Open Graph information. To customize preferred Google+ or Twitter Card data, use tabs above.</span></a>', 'seo-ultimate-pro' ) . "</label>\n<div class='col-sm-4 col-md-4'><input name='$id' id='$id' type='text' value='$value' class='form-control input-sm regular-text' tabindex='2'"
				. " onkeyup=\"javascript:document.getElementById('su_og_title_charcount').innerHTML = document.getElementById('_su_og_title').value.length\" />"
				. "</div>\n<div class='col-sm-4 col-md-4 help-text'>" . sprintf( __( 'You&#8217;ve Entered %s Characters. It&#8217;s Best to Stay Between 60 and 90 Characters.', 'seo-ultimate-pro' ), "<strong id='su_og_title_charcount'>" . strlen( $value ) . "</strong>" )
				. "</div>\n</div>\n";

			$id                                       = '_su_og_description';
			$value                                    = su_esc_attr( $this->get_postmeta( 'og_description' ) );
			$fields['facebook'][51]['og_description'] =
				"<div class='form-group su textarea'>\n<label class='col-sm-4 col-md-4 control-label' for='$id'>" . __( 'Open Graph Description: <a class="tooltips"> <img class="og-info-icon" src="' . plugins_url( 'opengraph/og-info-icon.png', dirname( __FILE__ ) ) . '" > <span>Facebook can display up to 300 characters, but we suggest a maximum of 200 characters.</span></a>', 'seo-ultimate-pro' ) . "</label>\n<div class='col-sm-4 col-md-4'>"
				. "<textarea name='$id' id='$id' class='form-control regular-text' cols='60' rows='3' tabindex='2'"
				. " onkeyup=\"javascript:document.getElementById('su_og_description_charcount').innerHTML = document.getElementById('_su_og_description').value.length\">$value</textarea>"
				. "</div>\n<div class='col-sm-4 col-md-4 help-text'>" . sprintf( __( 'You&#8217;ve Entered %s Characters. Keep Description to 200 Characters Max.', 'seo-ultimate-pro' ), "<strong id='su_og_description_charcount'>" . strlen( $value ) . "</strong>" )
				. "</div>\n</div>\n";

			$fields['facebook'][52]['og_image'] = $this->get_postmeta_medialib_box( 'og_image', __( 'Open Graph Image: <a class="tooltips"> <img class="og-info-icon" src="' . plugins_url( 'opengraph/og-info-icon.png', dirname( __FILE__ ) ) . '" > <span>Use image at least 1200 X 630 for best display on high resolution devices. Minimum recommended image is 600 x 315 pixels.</span></a>', 'seo-ultimate-pro' ) );

			$fields['facebook'][53]['og_type'] = $this->get_postmeta_dropdown( 'og_type', array(
				''                                   => __( 'Use default', 'seo-ultimate-pro' )
			,
				'none'                               => __( 'None', 'seo-ultimate-pro' )
			,
				'blog'                               => __( 'Blog', 'seo-ultimate-pro' )
			,
				'books'                              => __( 'Books', 'seo-ultimate-pro' )
			,
				'business'                           => __( 'Business', 'seo-ultimate-pro' )
			,
				'product'                            => __( 'Product', 'seo-ultimate-pro' )
			,
				'place'                              => __( 'Place', 'seo-ultimate-pro' )
			,
				'website'                            => __( 'Website', 'seo-ultimate-pro' )
			,
				__( 'Internet', 'seo-ultimate-pro' ) => array(
					'article' => __( 'Article', 'seo-ultimate-pro' )
				,
					'profile' => __( 'Profile', 'seo-ultimate-pro' )
				),
				__( 'Music', 'seo-ultimate-pro' )    => array(
					'music.album'         => __( 'Album', 'seo-ultimate-pro' )
				,
					'music.playlist'      => __( 'Playlist', 'seo-ultimate-pro' )
				,
					'music.radio_station' => __( 'Radio Station', 'seo-ultimate-pro' )
				,
					'music.song'          => __( 'Song', 'seo-ultimate-pro' )
				),
				__( 'Videos', 'seo-ultimate-pro' )   => array(
					'videoepisode'  => __( 'TV Episode', 'seo-ultimate-pro' )
				,
					'video.movie'   => __( 'Movie', 'seo-ultimate-pro' )
				,
					'video.other'   => __( 'Video', 'seo-ultimate-pro' )
				,
					'video.tv_show' => __( 'TV Show', 'seo-ultimate-pro' )
				)
			), __( 'Open Graph Content Type:', 'seo-ultimate-pro' ), array( 'help_text' => '' ) );

			$fields['facebook'][54]['og_books_isbn'] =
				$this->get_postmeta_subsection( 'og_type', 'books',
					$this->get_postmeta_textbox( 'og_books_isbn', __( 'ISBN: <a class="tooltips"> <img class="og-info-icon" src="' . plugins_url( 'opengraph/og-info-icon.png', dirname( __FILE__ ) ) . '" > <span>The International Standard Book Number (ISBN) for the book.</span></a>', 'seo-ultimate-pro' ), array( 'type' => 'text' ) )
				);
			...
			$fields['facebook'][55]['og_business_address|og_business_locality|og_business_postal_code|og_business_country|og_business_latitude|og_business_longitude'] =
				$this->get_postmeta_subsection( 'og_type', 'business',
					$this->get_postmeta_textbox( 'og_business_address', __( 'Street Address: <a class="tooltips"> <img class="og-info-icon" src="' . plugins_url( 'opengraph/og-info-icon.png', dirname( __FILE__ ) ) . '" > <span>The number and street of the postal address for this business. For example, 1600 Amphitheatre Pkwy.</span></a>', 'seo-ultimate-pro' ), array( 'type' => 'text' ) )
					. $this->get_postmeta_textbox( 'og_business_locality', __( 'Business City: <a class="tooltips"> <img class="og-info-icon" src="' . plugins_url( 'opengraph/og-info-icon.png', dirname( __FILE__ ) ) . '" > <span>The city (or locality) line of the postal address for this business. For example, Mountain View.</span></a>', 'seo-ultimate-pro' ), array( 'type' => 'text' ) )
					. $this->get_postmeta_textbox( 'og_business_postal_code', __( 'Postal Code: <a class="tooltips"> <img class="og-info-icon" src="' . plugins_url( 'opengraph/og-info-icon.png', dirname( __FILE__ ) ) . '" > <span>The postcode (or ZIP code) of the postal address for this business. For example, 94043.</span></a>', 'seo-ultimate-pro' ), array( 'type' => 'text' ) )
					. $this->get_postmeta_textbox( 'og_business_country', __( 'Country Name: <a class="tooltips"> <img class="og-info-icon" src="' . plugins_url( 'opengraph/og-info-icon.png', dirname( __FILE__ ) ) . '" > <span>The country of the postal address for this business. For example, USA.</span></a>', 'seo-ultimate-pro' ), array( 'type' => 'text' ) )
					. $this->get_postmeta_textbox( 'og_business_latitude', __( 'Latitude: <a class="tooltips" href="https://maps.google.com/" target="_blank"> <img class="og-info-icon" src="' . plugins_url( 'opengraph/og-info-icon.png', dirname( __FILE__ ) ) . '" > <span>The latitude of the business place. For example, 40.75. Click to open maps page in new tab.</span></a>', 'seo-ultimate-pro' ), array( 'type' => 'text' ) )
					. $this->get_postmeta_textbox( 'og_business_longitude', __( 'Longitude: <a class="tooltips" href="https://maps.google.com/" target="_blank"> <img class="og-info-icon" src="' . plugins_url( 'opengraph/og-info-icon.png', dirname( __FILE__ ) ) . '" > <span>The longitude of the business place. For example, 73.98. Click to open maps page in new tab.</span></a>', 'seo-ultimate-pro' ), array( 'type' => 'text' ) )
				);
			...
			$fields['facebook'][56]['og_product_original_price_amount|og_product_original_price_currency|og_product_pretax_price_amount|og_product_pretax_price_currency|og_product_price_amount|og_product_price_currency|og_product_shipping_cost_amount|og_product_shipping_cost_currency|og_product_weight_value|og_product_weight_units|og_product_shipping_weight_value|og_product_shipping_weight_units|og_product_sale_price_amount|og_product_sale_price_currency|og_product_sale_price_dates_start|og_product_sale_price_dates_end'] =
				$this->get_postmeta_subsection( 'og_type', 'product',
					...
					. $this->get_postmeta_textbox( 'og_product_price_amount', __( 'Product Price: <a class="tooltips"> <img class="og-info-icon" src="' . plugins_url( 'opengraph/og-info-icon.png', dirname( __FILE__ ) ) . '" > <span>The price of the product. For example 50 (See Price Currency Below For Additional Options).</span></a>', 'seo-ultimate-pro' ), array( 'type' => 'text' ) )
					...
					. $this->get_postmeta_textbox( 'og_product_weight_value', __( 'Product Weight: <a class="tooltips"> <img class="og-info-icon" src="' . plugins_url( 'opengraph/og-info-icon.png', dirname( __FILE__ ) ) . '" > <span>The weight of the product. For example, 7.</span></a>', 'seo-ultimate-pro' ), array( 'type' => 'text' ) )
					. $this->get_postmeta_textbox( 'og_product_weight_units', __( 'Product Weight Units: <a class="tooltips"> <img class="og-info-icon" src="' . plugins_url( 'opengraph/og-info-icon.png', dirname( __FILE__ ) ) . '" > <span>The units of the weight of the product. For example, Kg.</span></a>', 'seo-ultimate-pro' ), array( 'type' => 'text' ) )
					. $this->get_postmeta_textbox( 'og_product_shipping_weight_value', __( 'Shipping Weight: <a class="tooltips"> <img class="og-info-icon" src="' . plugins_url( 'opengraph/og-info-icon.png', dirname( __FILE__ ) ) . '" > <span>The shipping weight of the product. For example, 2.</span></a>', 'seo-ultimate-pro' ), array( 'type' => 'text' ) )
					. $this->get_postmeta_textbox( 'og_product_shipping_weight_units', __( 'Shipping Weight Units: <a class="tooltips"> <img class="og-info-icon" src="' . plugins_url( 'opengraph/og-info-icon.png', dirname( __FILE__ ) ) . '" > <span>The units of the shipping weight of the product. For example, Kg.</span></a>', 'seo-ultimate-pro' ), array( 'type' => 'text' ) )
			...
			$fields['facebook'][57]['og_place_latitude|og_place_longitude'] =
				$this->get_postmeta_subsection( 'og_type', 'place',
					$this->get_postmeta_textbox( 'og_place_latitude', __( 'Latitude: <a class="tooltips" href="https://maps.google.com/" target="_blank"> <img class="og-info-icon" src="' . plugins_url( 'opengraph/og-info-icon.png', dirname( __FILE__ ) ) . '" > <span>The latitude of the place. For example, 40.75. Click to open maps page in new tab.</span></a>', 'seo-ultimate-pro' ), array( 'type' => 'text' ) )
					. $this->get_postmeta_textbox( 'og_place_longitude', __( 'Longitude: <a class="tooltips" href="https://maps.google.com/" target="_blank"> <img class="og-info-icon" src="' . plugins_url( 'opengraph/og-info-icon.png', dirname( __FILE__ ) ) . '" > <span>The longitude of the place. For example, 73.98. Click to open maps page in new tab.</span></a>', 'seo-ultimate-pro' ), array( 'type' => 'text' ) )
				);
			...
			$fields['facebook'][58]['og_profile_first_name|og_profile_last_name|og_profile_gender|og_profile_username'] =
				$this->get_postmeta_subsection( 'og_type', 'profile',
					$this->get_postmeta_textbox( 'og_profile_first_name', __( 'First Name: <a class="tooltips"> <img class="og-info-icon" src="' . plugins_url( 'opengraph/og-info-icon.png', dirname( __FILE__ ) ) . '" > <span>The first name of the person that this profile represents.</span></a>', 'seo-ultimate-pro' ), array( 'type' => 'text' ) )
					. $this->get_postmeta_textbox( 'og_profile_last_name', __( 'Last Name: <a class="tooltips"> <img class="og-info-icon" src="' . plugins_url( 'opengraph/og-info-icon.png', dirname( __FILE__ ) ) . '" > <span>The last name of the person that this profile represents.</span></a>', 'seo-ultimate-pro' ), array( 'type' => 'text' ) )
					. $this->get_postmeta_textbox( 'og_profile_gender', __( 'Gender: <a class="tooltips"> <img class="og-info-icon" src="' . plugins_url( 'opengraph/og-info-icon.png', dirname( __FILE__ ) ) . '" > <span>The gender of the person that this profile represents.</span></a>', 'seo-ultimate-pro' ), array( 'type' => 'text' ) )
					. $this->get_postmeta_textbox( 'og_profile_username', __( 'User Name: <a class="tooltips"> <img class="og-info-icon" src="' . plugins_url( 'opengraph/og-info-icon.png', dirname( __FILE__ ) ) . '" > <span>A username for the person that this profile represents.</span></a>', 'seo-ultimate-pro' ), array( 'type' => 'text' ) )
				);
			...
			$fields['facebook'][59]['og_video_episode_url|og_video_episode_secure_url|og_video_episode_type|og_video_episode_width|og_video_episode_height|og_video_episode_actor_id|og_video_episode_actor_role|og_video_episode_director|og_video_episode_duration|og_video_episode_release_date|og_video_episode_tags|og_video_episode_writer'] =
				$this->get_postmeta_subsection( 'og_type', 'videoepisode',
					$this->get_postmeta_textbox( 'og_video_episode_url', __( 'Video URL: <a class="tooltips"> <img class="og-info-icon" src="' . plugins_url( 'opengraph/og-info-icon.png', dirname( __FILE__ ) ) . '" > <span>The URL of a video resource associated with the object.</span></a>', 'seo-ultimate-pro' ), array( 'type' => 'text' ) )
					. $this->get_postmeta_textbox( 'og_video_episode_secure_url', __( 'Video Secure URL: <a class="tooltips"> <img class="og-info-icon" src="' . plugins_url( 'opengraph/og-info-icon.png', dirname( __FILE__ ) ) . '" > <span>An alternate URL to use if a video resource requires HTTPS.</span></a>', 'seo-ultimate-pro' ), array( 'type' => 'text' ) )
					. $this->get_postmeta_textbox( 'og_video_episode_type', __( 'Video Type: <a class="tooltips"> <img class="og-info-icon" src="' . plugins_url( 'opengraph/og-info-icon.png', dirname( __FILE__ ) ) . '" > <span>The MIME type of a video resource associated with the object.</span></a>', 'seo-ultimate-pro' ), array( 'type' => 'text' ) )
					. $this->get_postmeta_textbox( 'og_video_episode_width', __( 'Video Width: <a class="tooltips"> <img class="og-info-icon" src="' . plugins_url( 'opengraph/og-info-icon.png', dirname( __FILE__ ) ) . '" > <span>The width of a video resource associated with the object in pixels.</span></a>', 'seo-ultimate-pro' ), array( 'type' => 'text' ) )
					. $this->get_postmeta_textbox( 'og_video_episode_height', __( 'Video Height: <a class="tooltips"> <img class="og-info-icon" src="' . plugins_url( 'opengraph/og-info-icon.png', dirname( __FILE__ ) ) . '" > <span>The height of a video resource associated with the object in pixels</span></a>', 'seo-ultimate-pro' ), array( 'type' => 'text' ) )
					. $this->get_postmeta_textbox( 'og_video_episode_actor_id', __( 'Actor ID: <a class="tooltips"> <img class="og-info-icon" src="' . plugins_url( 'opengraph/og-info-icon.png', dirname( __FILE__ ) ) . '" > <span>The Facebook IDs (or references to the profiles) of the actors in the movie.</span></a>', 'seo-ultimate-pro' ), array( 'type' => 'text' ) )
					. $this->get_postmeta_textbox( 'og_video_episode_actor_role', __( 'Actor Role: <a class="tooltips"> <img class="og-info-icon" src="' . plugins_url( 'opengraph/og-info-icon.png', dirname( __FILE__ ) ) . '" > <span>The roles played by the actors in the movie.</span></a>', 'seo-ultimate-pro' ), array( 'type' => 'text' ) )
					. $this->get_postmeta_textbox( 'og_video_episode_director', __( 'Director: <a class="tooltips"> <img class="og-info-icon" src="' . plugins_url( 'opengraph/og-info-icon.png', dirname( __FILE__ ) ) . '" > <span>The Facebook IDs (or references to the profiles) of the directors of the movie.</span></a>', 'seo-ultimate-pro' ), array( 'type' => 'text' ) )
					. $this->get_postmeta_textbox( 'og_video_episode_duration', __( 'Duration: <a class="tooltips"> <img class="og-info-icon" src="' . plugins_url( 'opengraph/og-info-icon.png', dirname( __FILE__ ) ) . '" > <span>An integer representing the length of the movie in seconds.</span></a>', 'seo-ultimate-pro' ), array( 'type' => 'text' ) )
					. $this->get_postmeta_textbox( 'og_video_episode_release_date', __( 'Release Date: <a class="tooltips"> <img class="og-info-icon" src="' . plugins_url( 'opengraph/og-info-icon.png', dirname( __FILE__ ) ) . '" > <span>A time representing when the movie was released.</span></a>', 'seo-ultimate-pro' ), array( 'type' => 'text' ) )
					. $this->get_postmeta_textbox( 'og_video_episode_tags', __( 'Tags: <a class="tooltips"> <img class="og-info-icon" src="' . plugins_url( 'opengraph/og-info-icon.png', dirname( __FILE__ ) ) . '" > <span>Keywords relevant to the movie.</span></a>', 'seo-ultimate-pro' ), array( 'type' => 'text' ) )
					. $this->get_postmeta_textbox( 'og_video_episode_writer', __( 'Writer: <a class="tooltips"> <img class="og-info-icon" src="' . plugins_url( 'opengraph/og-info-icon.png', dirname( __FILE__ ) ) . '" > <span>The Facebook IDs (or references to the profiles) of the writers of the movie.</span></a>', 'seo-ultimate-pro' ), array( 'type' => 'text' ) )
				);

			$id                                  = "_su_fb_app_id";
			$value                               = su_esc_attr( $this->get_postmeta( 'fb_app_id' ) );
			$fields['facebook'][80]['fb_app_id'] =
				"<div class='form-group su textbox'>\n<label class='col-sm-4 col-md-4 control-label' for='$id'>" . __( 'Facebook App ID:', 'seo-ultimate-pro' ) . "</label>\n<div class='col-sm-4 col-md-4'><input name='$id' id='$id' type='text' value='$value' class='form-control input-sm regular-text' tabindex='2'"
				. " />"
				. "</div>\n<div class='col-sm-4 col-md-4 help-text'>" . sprintf( __( 'Leave blank to pull from global or enter Facebook admin name/number (located under Facebook General Account Settings > Username). If more than one admin exists, enter all separated by commas.', 'seo-ultimate-pro' ) )
				. "</div>\n</div>\n";

			$id                                      = "_su_fb_admins_url";
			$value                                   = su_esc_attr( $this->get_postmeta( 'fb_admins_url' ) );
			$fields['facebook'][81]['fb_admins_url'] =
				"<div class='form-group su textbox'>\n<label class='col-sm-4 col-md-4 control-label' for='$id'>" . __( 'Facebook Admins:', 'seo-ultimate-pro' ) . "</label>\n<div class='col-sm-4 col-md-4'><input name='$id' id='$id' type='text' value='$value' class='form-control input-sm regular-text' tabindex='2'"
				. " />"
				. "</div>\n<div class='col-sm-4 col-md-4 help-text'>" . sprintf( __( 'Enter user id of facebook fan page admin. If more than one admin exists, enter all the id\'s separated by comma for facebook insights.', 'seo-ultimate-pro' ) )
				. "</div>\n</div>\n";

			$fields['twitter'][60]['twitter_card'] = $this->get_postmeta_dropdown( 'twitter_card', array(
				''                    => __( 'Use default', 'seo-ultimate-pro' )
			,
				'summary'             => __( 'Summary', 'seo-ultimate-pro' )
			,
				'summary_large_image' => __( 'Summary Large Image', 'seo-ultimate-pro' )
			,
				'app'                 => __( 'App', 'seo-ultimate-pro' )
			,
				'player'              => __( 'Player', 'seo-ultimate-pro' )
			), __( 'Twitter Card Type:', 'seo-ultimate-pro' ) );

			$id                                            = "_su_twitter_title_handle";
			$value                                         = su_esc_attr( $this->get_postmeta( 'twitter_title_handle' ) );
			$fields['twitter'][61]['twitter_title_handle'] =
				"<div class='form-group su textbox'>\n<label class='col-sm-4 col-md-4 control-label' for='$id'>" . __( '<a class="tooltips">Twitter Title:<span>Title should be concise and will be truncated at 70 characters.</span></a>', 'seo-ultimate-pro' ) . "</label>\n<div class='col-sm-4 col-md-4'><input name='$id' id='$id' type='text' value='$value' class='form-control input-sm regular-text' tabindex='2'"
				. " onkeyup=\"javascript:document.getElementById('su_twitter_title_handle_charcount').innerHTML = document.getElementById('_su_twitter_title_handle').value.length\" />"
				. "</div>\n<div class='col-sm-4 col-md-4 help-text'>" . sprintf( __( 'You&#8217;ve Entered %s Characters. Twitter Displays Up To 70 Characters.', 'seo-ultimate-pro' ), "<strong id='su_twitter_title_handle_charcount'>" . strlen( $value ) . "</strong>" )
				. "</div>\n</div>\n";

			$id                                                  = '_su_twitter_description_handle';
			$value                                               = su_esc_attr( $this->get_postmeta( 'twitter_description_handle' ) );
			$fields['twitter'][62]['twitter_description_handle'] =
				"<div class='form-group su textarea'>\n<label class='col-sm-4 col-md-4 control-label' for='$id'>" . __( '<a class="tooltips">Twitter Description:<span>Description of content (maximum 200 characters).</span></a>', 'seo-ultimate-pro' ) . "</label>\n<div class='col-sm-4 col-md-4'>"
				. "<textarea name='$id' id='$id' class='form-control regular-text' cols='60' rows='3' tabindex='2'"
				. " onkeyup=\"javascript:document.getElementById('su_twitter_description_handle_charcount').innerHTML = document.getElementById('_su_twitter_description_handle').value.length\">$value</textarea>"
				. "</div>\n<div class='col-sm-4 col-md-4 help-text'>" . sprintf( __( 'You&#8217;ve Entered %s Characters. Maximum Suggested Twitter Description is 200 Characters or Less.', 'seo-ultimate-pro' ), "<strong id='su_twitter_description_handle_charcount'>" . strlen( $value ) . "</strong>" )
				. "</div>\n</div>\n";

			$fields['twitter'][63]['twitter_image_handle'] = $this->get_postmeta_medialib_box( 'twitter_image_handle', __( '<a class="tooltips">Twitter Image:<span>Use button to upload image. It must be less than 1MB in size.</span></a>', 'seo-ultimate-pro' ) );
			...
			$fields['twitter'][68]['twitter_app_name_iphone_handle|twitter_app_id_iphone_handle|twitter_app_url_iphone_handle|twitter_app_name_ipad_handle|twitter_app_id_ipad_handle|twitter_app_url_ipad_handle|twitter_app_name_googleplay_handle|twitter_app_id_googleplay_handle|twitter_app_url_googleplay_handle'] =
				$this->get_postmeta_subsection( 'twitter_card', 'app',
					$this->get_postmeta_textbox( 'twitter_app_name_iphone_handle', __( '<a class="tooltips">Twitter App Name iPhone:<span>Name of your iPhone app.</span></a>', 'seo-ultimate-pro' ), array( 'type' => 'text' ) )
					. $this->get_postmeta_textbox( 'twitter_app_id_iphone_handle', __( '<a class="tooltips">Twitter App ID iPhone:<span>Your app ID in the iTunes App Store (Note: NOT your bundle ID).</span></a>', 'seo-ultimate-pro' ), array( 'type' => 'text' ) )
					. $this->get_postmeta_textbox( 'twitter_app_url_iphone_handle', __( '<a class="tooltips">Twitter App URL iPhone:<span>Your app\'s custom URL scheme (you must include "://" after your scheme name).</span></a>', 'seo-ultimate-pro' ), array( 'type' => 'text' ) )
					. $this->get_postmeta_textbox( 'twitter_app_name_ipad_handle', __( '<a class="tooltips">Twitter App Name iPad:<span>Name of your iPad optimized app.</span></a>', 'seo-ultimate-pro' ), array( 'type' => 'text' ) )
					. $this->get_postmeta_textbox( 'twitter_app_id_ipad_handle', __( '<a class="tooltips">Twitter App ID iPad:<span>Your app ID in the iTunes App Store.</span></a>', 'seo-ultimate-pro' ), array( 'type' => 'text' ) )
					. $this->get_postmeta_textbox( 'twitter_app_url_ipad_handle', __( '<a class="tooltips">Twitter App URL iPad:<span>Your app&#39;s custom URL scheme.</span></a>', 'seo-ultimate-pro' ), array( 'type' => 'text' ) )
					. $this->get_postmeta_textbox( 'twitter_app_name_googleplay_handle', __( '<a class="tooltips">Twitter App Name Googleplay:<span>Name of your Android app.</span></a>', 'seo-ultimate-pro' ), array( 'type' => 'text' ) )
					. $this->get_postmeta_textbox( 'twitter_app_id_googleplay_handle', __( '<a class="tooltips">Twitter App ID Googleplay:<span>Your app ID in the Google Play Store.</span></a>', 'seo-ultimate-pro' ), array( 'type' => 'text' ) )
					. $this->get_postmeta_textbox( 'twitter_app_url_googleplay_handle', __( '<a class="tooltips">Twitter App URL Googleplay:<span>Your app&#39;s custom URL scheme.</span></a>', 'seo-ultimate-pro' ), array( 'type' => 'text' ) )
				);
			...
			$fields['twitter'][69]['twitter_player_handle|twitter_player_width_handle|twitter_player_height_handle|twitter_image_handle|twitter_player_stream_handle|twitter_player_stream_content_type_handle'] =
				$this->get_postmeta_subsection( 'twitter_card', 'player',
					$this->get_postmeta_textbox( 'twitter_player_handle', __( '<a class="tooltips">Twitter Player:<span>HTTPS URL of player iframe.</span></a>', 'seo-ultimate-pro' ), array( 'type' => 'text' ) )
					. $this->get_postmeta_textbox( 'twitter_player_width_handle', __( '<a class="tooltips">Player Width:<span>Width of iframe in pixels.</span></a>', 'seo-ultimate-pro' ), array( 'type' => 'text' ) )
					. $this->get_postmeta_textbox( 'twitter_player_height_handle', __( '<a class="tooltips">Player Height:<span>Height of iframe in pixels.</span></a>', 'seo-ultimate-pro' ), array( 'type' => 'text' ) )
					. $this->get_postmeta_textbox( 'twitter_player_stream_handle', __( '<a class="tooltips">Player Stream:<span>URL to raw video or audio stream.</span></a>', 'seo-ultimate-pro' ), array( 'type' => 'text' ) )
					. $this->get_postmeta_textbox( 'twitter_player_stream_content_type_handle', __( '<a class="tooltips">Player Stream Content Type:<span>The MIME type/subtype combination that describes the content contained in twitter:player:stream. Takes the form specified in RFC 6381. Currently supported content_type values are those defined in RFC 4337 (MIME Type Registration for MP4).</span></a>', 'seo-ultimate-pro' ), array( 'type' => 'text' ) )
				);

			$id                                       = "_su_gplus_title";
			$value                                    = su_esc_attr( $this->get_postmeta( 'gplus_title' ) );
			$fields['google-plus'][71]['gplus_title'] =
				"<div class='form-group su textbox'>\n<label class='col-sm-4 col-md-4 control-label' for='$id'>" . __( 'Google Plus Title:', 'seo-ultimate-pro' ) . "</label>\n<div class='col-sm-4 col-md-4'><input name='$id' id='$id' type='text' value='$value' class='form-control input-sm regular-text' tabindex='2'"
				. " onkeyup=\"javascript:document.getElementById('su_gplus_title_charcount').innerHTML = document.getElementById('_su_gplus_title').value.length\" />"
				. "</div>\n<div class='col-sm-4 col-md-4 help-text'>" . sprintf( __( 'You&#8217;ve Entered %s Characters. Title Should Not Exceed 60 Characters.', 'seo-ultimate-pro' ), "<strong id='su_gplus_title_charcount'>" . strlen( $value ) . "</strong>" )
				. "</div>\n</div>\n";

			$id                                             = '_su_gplus_description';
			$value                                          = su_esc_attr( $this->get_postmeta( 'gplus_description' ) );
			$fields['google-plus'][72]['gplus_description'] =
				"<div class='form-group su textarea'>\n<label class='col-sm-4 col-md-4 control-label' for='$id'>" . __( 'Google Plus Description:', 'seo-ultimate-pro' ) . "</label>\n<div class='col-sm-4 col-md-4'>"
				. "<textarea name='$id' id='$id' class='form-control regular-text' cols='60' rows='3' tabindex='2'"
				. " onkeyup=\"javascript:document.getElementById('su_gplus_description_charcount').innerHTML = document.getElementById('_su_gplus_description').value.length\">$value</textarea>"
				. "</div>\n<div class='col-sm-4 col-md-4 help-text'>" . sprintf( __( 'You&#8217;ve Entered %s Characters. Description Should Not Exceed 156 Characters.', 'seo-ultimate-pro' ), "<strong id='su_gplus_description_charcount'>" . strlen( $value ) . "</strong>" )
				. "</div>\n</div>\n";

			$fields['google-plus'][73]['gplus_image_src'] = $this->get_postmeta_medialib_box( 'gplus_image_src', __( '<a class="tooltips">Google Plus Image:<span>The height must be at least 120px, and if the width is less than 100px, then the aspect ratio must be no greater than 3.0.</span></a>', 'seo-ultimate-pro' ), 'types=posttype_attachment&post_mime_type=image/*' );

			$id                                          = "_su_gplus_page_url";
			$value                                       = su_esc_attr( $this->get_postmeta( 'gplus_page_url' ) );
			$fields['google-plus'][74]['gplus_page_url'] =
				"<div class='form-group su textbox'>\n<label class='col-sm-4 col-md-4 control-label' for='$id'>" . __( 'Google Plus Publisher:', 'seo-ultimate-pro' ) . "</label>\n<div class='col-sm-4 col-md-4'><input name='$id' id='$id' type='text' value='$value' class='form-control input-sm regular-text' tabindex='2'"
				. " />"
				. "</div>\n<div class='col-sm-4 col-md-4 help-text'>" . sprintf( __( 'Enter your Google Plus page URL here if you have set up a "Google+ Page" for your organization or product, and the plugin will put a rel="publisher" link to the specified Google+ page on your page.', 'seo-ultimate-pro' ) )
				. "</div>\n</div>\n";

			$fields['advanced'][28]['previous_url_tag'] = $this->get_postmeta_textbox( 'previous_url_tag', __( 'Previous URL: <a class="tooltips"> <img class="og-info-icon" src="' . plugins_url( 'opengraph/og-info-icon.png', dirname( __FILE__ ) ) . '" > <span>The first page only contains rel="next" and no rel="prev" markup. Pages two to the second-to-last page should be doubly-linked with both rel="next" and rel="prev" markup. The last page only contains markup for rel="prev", not rel="next".</span></a>', 'seo-ultimate-pro' ), array( 'type' => 'text' ) );
			...
			$fields['advanced'][31]['meta_robots_opengraph'] = $this->get_postmeta_checkboxes( array(
				'meta_robots_opengraph' => __( 'Don\'t add Open Graph.', 'seo-ultimate-pro' )
			), __( 'Open Graph:', 'seo-ultimate-pro' ) );
		...
		function get_jlsuggest_box( $name, $value, $params = '', $placeholder = '' ) {
			...
			if ( empty( $value ) && $this->jlsuggest_box_post_id && current_theme_supports( 'post-thumbnails' ) && $thumbnail_id = get_post_thumbnail_id( $this->jlsuggest_box_post_id ) ) {
				...
				$placeholder   = sprintf( __( 'Featured Image: %s', 'seo-ultimate-pro' ), $selected_post->post_title );
			}
		...
		function get_type_options() {
			...
			,
				...
				__( 'Books', 'seo-ultimate-pro' )    => array(
					'books.book'   => __( 'Book', 'seo-ultimate-pro' )
				,
					'books.author' => __( 'Author', 'seo-ultimate-pro' )
				,
					'books.genre'  => __( 'Genre', 'seo-ultimate-pro' )
				),
				__( 'Music', 'seo-ultimate-pro' )    => array(
					'music.album'         => __( 'Album', 'seo-ultimate-pro' )
				,
				...
				__( 'Videos', 'seo-ultimate-pro' )   => array(
					'video.movie'   => __( 'Movie', 'seo-ultimate-pro' )
				,
					'video.episode' => __( 'TV Episode', 'seo-ultimate-pro' )
				,
					'video.tv_show' => __( 'TV Show', 'seo-ultimate-pro' )
				,
					'video.other'   => __( 'Video', 'seo-ultimate-pro' )
		...
		function add_twitter_field( $contactmethods ) {
			$contactmethods['twitter'] = __( 'Twitter ID', 'seo-ultimate-pro' );
		...
		function user_meta_fields( $user ) {
			?>
            <!--
            <h3><?php _e( "SEO Ultimate Settings", "blank" ); ?></h3>
            <table class="form-table">
                <tr>
                    <th>
                        <label
                                for="author_metatitle"><?php _e( "Author Meta Title", 'seo-ultimate-pro' ); ?></label>
                    </th>
                    <td>
                        <input class="regular-text" type="text" id="author_metatitle" name="author_metatitle"
                               value="<?php echo esc_attr( get_the_author_meta( 'author_title', $user->ID ) ); ?>"/>
                    </td>
                </tr>
                <tr>
                    <th>
                        <label
                                for="author_metadesc"><?php _e( 'Author Meta Description', 'seo-ultimate-pro' ); ?></label>
                    </th>
                    <td>
                        <textarea rows="3" cols="30" id="author_metadesc"
                                  name="author_metadesc"><?php echo esc_textarea( get_the_author_meta( 'author_desc', $user->ID ) ); ?></textarea>
                    </td>
                </tr>
            </table> -->
			<?php
