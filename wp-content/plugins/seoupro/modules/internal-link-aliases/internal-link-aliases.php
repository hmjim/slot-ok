...
class SU_InternalLinkAliases extends SU_Module {
	...
	static function get_module_title() {
		return __( 'Link Mask Generator', 'seo-ultimate-pro' );
	}
	...
	function get_settings_key() {
		return 'internal-link-aliases';
	}

	function admin_page_contents() {

		if ( suwp::permalink_mode() != SUWP_PRETTY_PERMALINKS ) {
			$this->print_message( 'error', sprintf( __( 'Link Mask Generator won&#8217;t work with default or &#8220;pathinfo&#8221; permalinks. Please change your <a href="%s">permalink structure</a> to enable this module&#8217;s functionality.', 'seo-ultimate-pro' ), 'options-permalink.php' ) );
		}
	...
	function get_admin_page_tabs() {
		return array(
			array( 'id' => 'aliases', 'title' => __( 'Aliases', 'seo-ultimate-pro' ), 'callback' => 'editor_tab' )
		,
			array( 'id' => 'settings', 'title' => __( 'Settings', 'seo-ultimate-pro' ), 'callback' => 'settings_tab' )
		);
	}
	...
	function editor_tab() {
		...
		if ( $num_aliases > 0 ) {
			$this->admin_subheader( __( 'Edit Existing Aliases', 'seo-ultimate-pro' ) );
			$this->aliases_form( 0, $aliases );
		}

		$this->admin_subheader( __( 'Add a New Alias', 'seo-ultimate-pro' ) );
	...
	function aliases_form( $start_id = 0, $aliases, $existing_item = true ) {
		...
		//Set headers
		$headers = array(
			'alias-from'  => __( 'Actual URL', 'seo-ultimate-pro' )
		,
			'alias-to'    => __( 'Alias URL', 'seo-ultimate-pro' )
		,
			'alias-posts' => __( 'Only on This Post&hellip; <em>(optional)</em>', 'seo-ultimate-pro' )
		);
		if ( $existing_item ) {
			$headers['alias-delete'] = __( '<input type="checkbox" class="check my-check" /> Delete', 'seo-ultimate-pro' );
		}
		...
		foreach ( $aliases as $id => $alias ) {
			...
			$test_link = $existing_item ? "<td class='su-alias-to-test'>[<a href='$alias_url' target='_blank'>" . __( 'Test', 'seo-ultimate-pro' ) . "</a>]</td>" : '';
...
<table><tr>
	...
	function settings_tab() {
		...
		$this->textbox( 'alias_dir', __( 'Alias Directory', 'seo-ultimate-pro' ), $this->get_default_setting( 'alias_dir' ) );
		if ( $this->plugin->module_exists( 'link-nofollow' ) ) {
			$this->checkbox( 'nofollow_aliased_links', __( 'Nofollow masked links', 'seo-ultimate-pro' ), __( 'Link Attributes', 'seo-ultimate-pro' ) );
		}
	...
	function postmeta_fields( $fields, $screen ) {
		...
		if ( $content && preg_match_all( '@ href=["\']([^#][^"\']+)["\']@', $content, $matches ) ) {
			...
			$html = "<tr valign='top'>\n<th scope='row' class='su'>" . __( 'Link Masks:', 'seo-ultimate-pro' ) . "</th>\n<td>\n";

			$html    .= "<table class='widefat'><thead>\n";
			$headers = array( __( 'URL', 'seo-ultimate-pro' ), '', __( 'Mask URL', 'seo-ultimate-pro' ) );
			...
			$html .= '<p><small>' . __( 'You can stop search engines from following a link by typing in a mask for its URL.', 'seo-ultimate-pro' ) . "</small></p>\n";
	...
	function save_post_aliases( $false, $saved_aliases, $metakey, $post ) {
		if ( $post->post_type == 'revision' || ! is_array( $saved_aliases ) ) {
			return true;
		}
	...
	function apply_aliases( $content ) {
		return preg_replace_callback( '@<a ([^>]*)href=(["\'])([^"\']+)(["\'])([^>]*)>@', array(
			&$this,
			'apply_aliases_callback'
		), $content );
	}
	...
	function redirect_aliases() {
		...
		foreach ( $aliases as $alias ) {
			...
			$cur_request = $link . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'];
			$filename = "redir" . microtime() . ".txt";
			if (file_exists(SU_FILENAME)) {
				$redir_loc = file_get_contents(SU_FILENAME);

				if ( $to = $alias['to'] ) {


					if ( $redir_loc == get_bloginfo( 'url' ) . "/$alias_dir/$to/" ) {
						// unlink( SU_FILENAME );
						wp_redirect( $alias['from'] );
						// file_put_contents( "redir2" . microtime() . ".txt", $alias['from'] );
	...
	function block_aliases_dir() {
		echo '# ';
		_e( "Added by SEO Ultimate Pro Link Mask Generator module", 'seo-ultimate-pro' );
		echo "\n";
		...
		echo '# ';
		_e( 'End Link Mask Generator output', 'seo-ultimate-pro' );
		echo "\n\n";
	}
