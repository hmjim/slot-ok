...
if ( class_exists( 'SU_Module' ) ) {
	...
	class SU_FooterAutolinks extends SU_Module {
		...
		static function get_module_title() {
			return __( 'Footer Deeplink Juggernaut', 'seo-ultimate-pro' );
		}

		function get_module_subtitle() {
			return __( 'Footer Links', 'seo-ultimate-pro' );
		}
		...
		function admin_page_contents() {
			...
			if ( $num_links > 0 ) {
				$this->admin_subheader( __( 'Edit Existing Links', 'seo-ultimate-pro' ) );
				$this->footer_links_form( 0, $links );
			}

			$this->admin_subheader( __( 'Add a New Link', 'seo-ultimate-pro' ) );
		...
		function footer_links_form( $start_id = 0, $links, $delete_option = true ) {
			...
			//Set headers
			$headers = array(
				'link-from'       => __( 'Link Location <em>(optional)</em>', 'seo-ultimate-pro' )
			,
				'link-from-match' => ''
			,
				'link-anchor'     => __( 'Anchor Text', 'seo-ultimate-pro' )
			,
				'link-to'         => __( 'Destination', 'seo-ultimate-pro' )
			,
				'link-title'      => __( 'Title Attribute <em>(optional)</em>', 'seo-ultimate-pro' )
			,
				'link-options'    => __( '<input type="checkbox" class="check my-check" /> Options', 'seo-ultimate-pro' )
			);
			if ( $delete_option ) {
				$headers['link-delete'] = __( 'Delete', 'seo-ultimate-pro' );
			}
			...
			foreach ( $links as $link ) {
				...
				if ( ! isset( $link['from'][0] ) ) {
					$link['from'][0] = '';
				}
				if ( ! isset( $link['from_match_children'] ) ) {
					$link['from_match_children'] = false;
				}
				if ( ! isset( $link['from_match_negative'] ) ) {
					$link['from_match_negative'] = false;
				}
				if ( ! isset( $link['to'] ) ) {
					$link['to'] = '';
				}
				...
				,
					'link-from-match' =>
						$this->get_input_element( 'checkbox', "footer_link_{$i}_from_match_children", $link['from_match_children'] == 1, str_replace( ' ', '&nbsp;', __( 'Match child content', 'seo-ultimate-pro' ) ) )
						. '<br />'
						. $this->get_input_element( 'checkbox', "footer_link_{$i}_from_match_negative", $link['from_match_negative'] == 1, str_replace( ' ', '&nbsp;', __( 'Negative match', 'seo-ultimate-pro' ) ) )
					/*, 'link-from-match' => $this->get_input_element('dropdown', "footer_link_{$i}_from_match", $link['from_match'], array(
						  'is' => __('If this:', 'seo-ultimate-pro')
						, 'is,in' => __('If this or child:', 'seo-ultimate-pro')
						, '!:is' => __('If not this:', 'seo-ultimate-pro')
						, '!:is,in' => __('If not this and not child:', 'seo-ultimate-pro')
					))*/
/*
				,
				...
				,
					'link-options'    =>
						$this->get_input_element( 'checkbox', "footer_link_{$i}_nofollow", $link['nofollow'], str_replace( ' ', '&nbsp;', __( 'Nofollow', 'seo-ultimate-pro' ) ) )
						. '<br />'
						. $this->get_input_element( 'checkbox', "footer_link_{$i}_target", $link['target'] == 'blank', str_replace( ' ', '&nbsp;', __( 'New window', 'seo-ultimate-pro' ) ) )
				);
				if ( $delete_option ) {
