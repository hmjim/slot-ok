...
if ( class_exists( 'SU_Module' ) ) {
	...
	class SU_Slugs extends SU_Module {
		...
		static function get_module_title() {
			return __( 'Slug Optimizer', 'seo-ultimate-pro' );
		}
		...
		function admin_page_contents() {
			...
			$this->textarea( 'words_to_remove', __( 'Words to Remove', 'seo-ultimate-pro' ), 20 );
		...
		function init() {
			...
			if ( isset( $_POST['action'] ) && $_POST['action'] == 'sample-permalink' ) //The filter priority is very important to ensure our function runs before WordPress's sanitize_title_with_dashes() function
			{
		...
		function optimize_slug_ajax( $title ) {
			...
			if ( strcmp( $title, $_POST['new_title'] ) == 0 ) //An empty slug was given, so the post title is being used as the default! Call to action!
			{
...
", 'seo-ultimate-pro' );
			...
			if ( $this->has_enabled_parent() ) {
				$screen->add_help_tab( array(
					'id'      => 'su-slugs-help'
				,
					'title'   => __( 'Slug Optimizer', 'seo-ultimate-pro' )
				,
					'content' =>
						'<h3>' . __( 'Overview', 'seo-ultimate-pro' ) . '</h3>' . $overview .
						'<h3>' . __( 'FAQ', 'seo-ultimate-pro' ) . '</h3>' . $faq .
						'<h3>' . __( 'Troubleshooting', 'seo-ultimate-pro' ) . '</h3>' . $troubleshooting
				) );
			} else {
