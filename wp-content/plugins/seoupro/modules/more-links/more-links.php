...
if ( class_exists( 'SU_Module' ) ) {
	...
	class SU_MoreLinks extends SU_Module {
		...
		static function get_module_title() {
			return __( 'More Link Customizer', 'seo-ultimate-pro' );
		}
		...
		function admin_page_contents() {
			...
			$this->textbox( 'default', __( 'Default More Link Text', 'seo-ultimate-pro' ), $this->get_default_setting( 'default' ) );
		...
		function postmeta_fields( $fields, $screen ) {
			...
			if ( strcmp( $screen, 'post' ) == 0 ) {
				$fields['links'][20]['morelinktext'] = $this->get_postmeta_textbox( 'morelinktext', __( 'Anchor Text of &#8220;More&#8221; Link:', 'seo-ultimate-pro' ) );
			}
...
<ul>
	<li>
		...
		<p>If you&#8217;ve already specified custom anchors via WordPress&#8217;s method, SEO Ultimate Pro will import those anchors automatically into the More Link Customizer.</p>
	</li>
</ul>
", 'seo-ultimate-pro' );
			...
			if ( $this->has_enabled_parent() ) {
				...
				,
					'content' =>
						...
						'<h3>' . __( 'FAQ', 'seo-ultimate-pro' ) . '</h3>' . $faq
				) );
			} else {
