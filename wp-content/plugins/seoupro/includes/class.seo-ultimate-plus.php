...
class SEO_Ultimate_Plus {
	...
	function __construct( $plugin_file ) {
		...
		*/
		...
		$psdata = get_option( 'seo_ultimate_plus', array() );
		if ( ! is_array( $psdata ) ) {
			$psdata = array();
		}
		...
		*/
		...
		add_action( 'init', array(
			&$this,
			'load_textdomain'
		), 0 ); //Run before widgets_init hook (wp-includes/default-widgets.php)
		...
		if ( is_admin() ) {
			...
			add_action( "in_plugin_update_message-{$this->plugin_basename}", array(
				&$this,
				'plugin_update_info'
			), 10, 2 );
	...
	*/
	/**
	 * Upgrades SEO Ultimate Pro to version 0.8.
	 *
	 * @since 0.8
	 */
	...
	function upgrade_to_08() {
			...
			if (($value = get_option("su_$option", false)) !== false) {
				...
				if ('settings' == $option) {
					foreach ((array)$value as $module => $module_data)
						update_option("seo_ultimate_plus_module_$module", $module_data);
				} else {
					$psdata[$option] = $value;
					$save = true;
				}
		...
		if ($save) update_option('seo_ultimate_plus', $psdata);
	}
	*/

	/**
	 * Upgrades SEO Ultimate Pro to version 4.0.
	 *
	 * @since 4.0
	 */
	...
	*/
	...
	/**
	 * Upgrades SEO Ultimate Pro to version 5.0.
	 *
	 * @since 5.0
	 */
	...
	*/
	...
	function copy_module_states( $copy ) {
		...
		if ( $save ) {
			update_option( 'seo_ultimate_plus', array() );
		}
	}
	...
	function uninstall() {
		...
		if ( ! empty( $psdata['modules'] ) ) {
			...
			foreach ( $module_keys as $module ) {
				delete_option( "seo_ultimate_plus_module_$module" );
			}
		}

		//Delete plugin data
		delete_option( 'seo_ultimate_plus' );
	}
	 ...
	 *
	 * SEO Ultimate Pro uses a modular system that allows functionality to be added and removed on-the-fly.
	 *
	 * @since 0.1
	 * @uses $plugin_dir_path
				...
				while ( $file = readdir( $subdir ) ) {
					...
					if ( suio::is_file( $file, $subdirpath, 'php' ) && $file != 'index.php' ) {
						...
						if ( class_exists( $class ) ) {
							...
							if ( ! isset( $oldmodules[ $module ] ) && @call_user_func( array(
									$class,
									'get_default_status'
								) ) == SU_MODULE_DISABLED ) {
	...
	function load_textdomain() {
		load_plugin_textdomain( 'seo-ultimate-pro', '', trailingslashit( plugin_basename( $this->plugin_dir_path ) ) . 'translations' );
	}
	...
	function log_redirect( $redirect_url, $status_code ) {
		if ( empty( $this->hit_redirect_trigger ) ) {
			$this->hit_redirect_trigger = 'wp_redirect';
		}
	...
	function log_redirect_canonical( $redirect_url ) {
		if ( empty( $this->hit_redirect_trigger ) ) {
			$this->hit_redirect_trigger = 'redirect_canonical';
		}
		...
		foreach ( $this->modules as $key => $x_module ) {
			...
			if ( $module->get_menu_title()
				...
				if ( $parent == 'seo' && ! $added_main_menu ) {
					...
					add_menu_page( __( 'SEO Ultimate Pro', 'seo-ultimate-pro' ), __( 'SEO', 'seo-ultimate-pro' ) . $main_count_code, 'manage_options', 'seo', array(), 'div' );
	...
	function key_to_hook( $key ) {
		switch ( $key ) {
			case $this->default_menu_module:
				return 'seo';
				break;
			case 'settings':
				return 'seo-ultimate-pro';
				break;
			default:
				return "su-$key";
				break;
	...
	function hook_to_key( $hook ) {
		switch ( $hook ) {
			case 'seo':
				return $this->default_menu_module;
				break;
			case 'seo-ultimate-pro':
				return 'settings';
				break;
			default:
				return substr( $hook, 3 );
				break;
		}
	}
	...
	function is_plugin_admin_page() {
		if ( is_admin() ) {
			...
			foreach ( $this->modules as $key => $module ) {
				if ( strcmp( $plugin_page, $this->key_to_hook( $key ) ) == 0 ) {
					return true;
				}
			}
		}
	...
	function admin_includes() {
		...
		*/
		...
		$sdf_admin_pages = array(
			'sdf',
			'sdf-settings',
			'sdf-silo',
			'sdf-silo-manual-builder',
			'sdf-header',
			'sdf-layout',
			'sdf-shortcode',
			'sdf-styles',
			...
			'su-canonical-url',
			'su-files',
			'su-internal-link-aliases',
			'su-meta-descriptions',
			'su-meta-keywords',
			'su-meta-robots',
			'su-opengraph',
			'seo-ultimate-pro',
		...
		if ( in_array( $current, $sdf_admin_pages ) ) {
			wp_register_style( 'seo-css-admin', $this->plugin_dir_url . 'assets/css/seo.admin.css', array(), null, 'screen' );
			wp_enqueue_style( 'seo-css-admin' );
			wp_register_style( 'sdf-bootstrap-admin', $this->plugin_dir_url . 'assets/sdf/bootstrap/css/bootstrap.admin.css', array(), null, 'screen' );
			wp_register_style( 'sdf-bootstrap-admin-theme', $this->plugin_dir_url . 'assets/sdf/bootstrap/css/bootstrap-theme.css', array(), null, 'screen' );
			wp_register_style( 'sdf-font-awesome', $this->plugin_dir_url . 'assets/sdf/bootstrap/css/all.min.css', array(), null, 'screen' );
			wp_register_style( 'sup-modules-css', $this->plugin_dir_url . 'modules/modules/modules.css', array(), null, 'screen' );
			wp_register_style( 'su-features-css', $this->plugin_dir_url . 'assets/css/su_features.css', array(), null, 'screen' );
			wp_enqueue_style( 'sdf-bootstrap-admin' );
			wp_enqueue_style( 'sdf-bootstrap-admin-theme' );
			wp_enqueue_style( 'sup-modules-css' );
			wp_enqueue_style( 'sdf-font-awesome' );
			wp_enqueue_style( 'su-features-css' );

			wp_register_script( 'sdf_bs_js_admin', $this->plugin_dir_url . 'assets/js/bootstrap.min.js', array( 'jquery' ), null, true );
			wp_register_script( 'media_upload_js', $this->plugin_dir_url . 'assets/sdf/sdf.media.upload.js', array( 'jquery' ), '' );
			wp_register_script( 'sdf-font-awesome-js', $this->plugin_dir_url . 'assets/js/all.min.js', array( 'jquery' ), null, true );
			wp_enqueue_script( 'sdf_bs_js_admin' );
			wp_enqueue_script( 'media_upload_js' );
			wp_enqueue_script( 'sdf-font-awesome-js' );
			wp_enqueue_media();
		...
		wp_register_style( 'seo-css-admin-icon', $this->plugin_dir_url . 'assets/css/front_icon.css', array(), null, 'screen' );
		wp_enqueue_style( 'seo-css-admin-icon' );

		$su_pro_features = array(
			'seoupro-license',
			'seo',
			'su-importexport',
			'su-schema',
		...
		if ( in_array( $current, $su_pro_features )  || in_array( $pagenow, $pages ) || in_array( $pagenow, array(
			'post.php',
			'post-new.php'
		) ) ) {
			wp_register_style( 'seo-css-admin', $this->plugin_dir_url . 'assets/css/seo.admin.css', array(), null, 'screen' );
			wp_enqueue_style( 'seo-css-admin' );
			wp_register_style( 'sdf-bootstrap-admin', $this->plugin_dir_url . 'assets/css/bootstrap.css', array(), null, 'screen' );
			wp_enqueue_style( 'sdf-bootstrap-admin' );
			wp_register_style( 'seo-css-boxes-admin', $this->plugin_dir_url . 'assets/css/features-boxed.css');

			wp_register_style( 'sdf-font-awesome', $this->plugin_dir_url . 'assets/css/all.min.css', array(), null, 'screen' );
			wp_enqueue_style( 'sdf-font-awesome' );
			wp_register_style( 'sdf-bootstrap-admin-theme', $this->plugin_dir_url . 'assets/css/bootstrap-theme.css', array(), null, 'screen' );
			wp_enqueue_style( 'sdf-bootstrap-admin-theme' );
			wp_register_script( 'sdf_bs_js_admin', $this->plugin_dir_url . 'assets/js/bootstrap.min.js', array( 'jquery' ), null, true );
			wp_enqueue_script( 'sdf_bs_js_admin' );
			// wp_register_script( 'su-global-js', $this->plugin_dir_url . 'assets/js/global.js', array( 'jquery' ), null, true );
			wp_enqueue_style( 'seo-css-boxes-admin' );
			// wp_enqueue_script( 'su-global-js' );
		}
		...
		if ( strlen( $pp ) ) {
			...
			foreach ( $this->modules as $key => $module ) {
				...
				if ( strcmp( $key, $pp ) == 0 ) //Output AJAX page var fix
				{
				...
				if ( strcmp( $key, $pp ) == 0 || strcmp( $module->get_parent_module(), $pp ) == 0 ) {
					...
					if ( ! $outputted_module_files ) {
						...
						$this->queue_js( 'modules', 'modules', array( 'jquery' ), array(
							'unloadConfirmMessage' => __( "It looks like you made changes to the settings of this SEO Ultimate Pro module. If you leave before saving, those changes will be lost.", 'seo-ultimate-pro' )
						) );
	...
	function admin_help() {
		...
		if ( $helparray ) {
			...
			,
				'title'   => __( 'SEO Ultimate Pro Settings', 'seo-ultimate-pro' )
			,
	...
	/**
	 * Notifies the user if he/she is using plugins whose functionality SEO Ultimate Pro replaces.
	 *
	 * @since 0.1
	...
	/**
	 * Outputs a table row notifying the user that he/she is using a plugin which may conflict with SEO Ultimate Pro.
	 *
	 * @since 0.1
	 */
	function plugin_page_notice( $file, $data, $context ) {
		if ( is_plugin_active( $file ) ) {
			...
			printf( __( '%1$s is known to cause conflicts with SEO Ultimate Pro. Please deactivate %1$s if you wish to continue using SEO Ultimate Pro.', 'seo-ultimate-pro' ), $data['Name'] );
			echo "</div></td></tr>\n";
		}
	}
	...
	function plugin_update_info( $plugin_data, $r ) {
		...
		if ( $r && $r->new_version && ! is_plugin_active( 'changelogger/changelogger.php' ) ) //If info on the new version is available...
		{
			if ( $info = $this->get_plugin_update_info( $r->new_version ) ) //Output the new-version info
			{
	...
	function get_plugin_update_info( $nv ) {
		...
		$change_labels = array(
			'module'       => array( __( 'new module', 'seo-ultimate-pro' ), __( 'new modules', 'seo-ultimate-pro' ) )
		,
			'feature'      => array( __( 'new feature', 'seo-ultimate-pro' ), __( 'new features', 'seo-ultimate-pro' ) )
		,
			'bugfix'       => array( __( 'bugfix', 'seo-ultimate-pro' ), __( 'bugfixes', 'seo-ultimate-pro' ) )
		,
			'improvement'  => array( __( 'improvement', 'seo-ultimate-pro' ), __( 'improvements', 'seo-ultimate-pro' ) )
		,
			'security'     => array(
				__( 'security fix', 'seo-ultimate-pro' ),
				__( 'security fixes', 'seo-ultimate-pro' )
			)
		,
			'new-lang'     => array(
				__( 'new language pack', 'seo-ultimate-pro' ),
				__( 'new language packs', 'seo-ultimate-pro' )
			)
		,
			'updated-lang' => array(
				__( 'language pack update', 'seo-ultimate-pro' ),
				__( 'language pack updates', 'seo-ultimate-pro' )
			)
		);
		...
		foreach ( $changes as $change_type => $changes_count ) {
			if ( is_string( $change_type ) && $changes_count > 0 ) {
				$nlchanges[] = sprintf( __( '%d %s', 'seo-ultimate-pro' ),
					number_format_i18n( $changes_count ),
					_n( $change_labels[ $change_type ][0], $change_labels[ $change_type ][1], $changes_count, 'seo-ultimate-pro' )
				);
			}
		}

		return sprintf( __( 'Upgrade now to get %s. %s.', 'seo-ultimate-pro' )
			, sustr::nl_implode( $nlchanges )
			, '<a href="plugin-install.php?tab=plugin-information&amp;plugin=seo-ultimate-pro&amp;section=changelog&amp;TB_iframe=true&amp;width=640&amp;height=530" class="thickbox">' . __( 'View changelog', 'seo-ultimate-pro' ) . '</a>'
		);
	}
	...
	function download_changelog() {
		...
		$plugin = plugins_api( 'plugin_information', array( 'slug' => 'seo-ultimate-pro' ) );
	...
	/**
	 * Provides WordPress with SEO Ultimate Pro custom update info notices.
	 *
	 * @since 2.1
	 */
	...
	/**
	 * Filters the list of plugin action links for SEO Ultimate Pro and adds links to certain modules if those modules are enabled.
	 *
	...
	function plugin_action_links( $actions ) {
		$su_actions = array(
			'uninstall' => __( 'Uninstall', 'seo-ultimate-pro' )
		);
	...
	/**
	 * Outputs a list of active modules on SEO Ultimate Pro plugin page listing.
	 *
	 * @since 2.1
	 */
	function plugin_row_meta_filter( $plugin_meta, $plugin_file ) {
		if ( $plugin_file == $this->plugin_basename ) {
			...
			if ( is_blog_admin() ) {
				$title = __( 'Active Modules: ', 'seo-ultimate-pro' );
			} else {
				$title = '';
			}
	...
	function private_blog_admin_notice() {
		echo "\n<div class='error'><p>";
		_e( '<strong>SEO Ultimate Pro Notice:</strong> Your blog is configured to block search engine spiders. To resolve this, <a href="options-reading.php" target="_blank">go to your Reading settings</a> and disable the &#8220;discourage search engines&#8221; option.', 'seo-ultimate-pro' );
		echo "</p></div>";
	}
	...
	function get_postmeta_tabs() {
		return array(
			'serp'     => __( 'General', 'seo-ultimate-pro' )
		,
			'facebook' => __( 'Facebook', 'seo-ultimate-pro' )
		,
			'twitter'  => __( 'Twitter', 'seo-ultimate-pro' )
		,
			'links'    => __( 'Links', 'seo-ultimate-pro' )
		,
			'advanced' => __( 'Advanced', 'seo-ultimate-pro' )
			//	, 'misc' => __('Miscellaneous', 'seo-ultimate-pro')
		);
	}
		...
		foreach ( $this->modules as $key => $module ) {
			...
			foreach ( $module_fields as $tab => $tab_fields ) {
				if ( isset( $tabs[ $tab ] ) ) {
					...
					if ( strpos( $tab, '|' ) === false ) {
						if ( ! isset( $fields['misc'][ $tab ] ) ) {
							$fields['misc'][ $tab ] = array();
						}
	...
	function add_postmeta_box() {
		...
		foreach ( $posttypes as $screen ) {
			...
			if ( $this->get_postmeta_array( $screen ) ) {
				add_meta_box( 'su_postmeta', __( 'SEO Ultimate Pro Settings', 'seo-ultimate-pro' ), @create_function( '', 'global $seo_ultimate_plus; $seo_ultimate_plus->show_postmeta_box("' . $screen . '");' ), $screen, 'normal', 'high' );
			}
		}
	}
	...
	function show_postmeta_box( $screen ) {
		...
		foreach ( $_tabs as $tab_id => $tab_title ) {
			if ( isset( $data[ $tab_id ] ) ) {
				$tabs[] = array(
					'title'    => $tab_title,
					'id'       => $tab_id,
					'callback' => array( 'postmeta_tab', $tab_id, $screen )
				);
			}
		}
		$this->tabs( $tabs );
		...
		if ( ! $this->is_sdf_active() ) {
			...
			printf( __( '%1$s %2$s by %3$s', 'seo-ultimate-pro' ),
				'<a href="' . SU_PLUGIN_URI . '" target="_blank">' . __( SU_PLUGIN_NAME, 'seo-ultimate-pro' ) . '</a>',
				SU_VERSION,
				'<a href="' . SU_AUTHOR_URI . '" target="_blank">' . __( SU_AUTHOR, 'seo-ultimate-pro' ) . '</a>'
			);
			echo '</p>';
		}
	...
	function save_postmeta_box( $post_id, $post ) {
		...
		if ( ! isset( $_REQUEST['_su_wpnonce'] ) || ! wp_verify_nonce( $_REQUEST['_su_wpnonce'], 'su-update-postmeta' ) ) {
			return;
		}
				...
				foreach ( $tab_pos as $fields => $html ) {
					...
					foreach ( $fields as $field ) {
						...
						if ( ! apply_filters( "su_custom_update_postmeta-$field", false, $value, $metakey, $post ) ) {
							if ( empty( $value ) ) //Delete the old value
							{
			...
			foreach ( $crondata as $key => $crons ) {
				if ( $remove_all || ! isset( $this->modules[ $key ] ) ) {
					foreach ( $crons as $data ) {
						wp_clear_scheduled_hook( $data[0] );
					}
	...
	function template_head() {
		...
		if ( get_option( 'blog_public' ) ) {
			$su_robots = array_unique( apply_filters( 'su_meta_robots', array() ) );
			$robots    = implode( ',', $su_robots );
	...
	/**
	 * Marks code with HTML comments identifying SEO Ultimate Pro, if the user has set this option.
	 *
	 * @since 2.7
	 */
	...
	function jlsuggest_autocomplete() {
		...
		if ( ( ! $include || in_array( 'home', $include ) ) && sustr::ihas( $_GET['q'], 'home' ) ) {
			$items[] = array( 'text' => __( 'Home', 'seo-ultimate-pro' ), 'isheader' => true );
			$items[] = array(
				'text'         => __( 'Blog Homepage', 'seo-ultimate-pro' ),
				'value'        => 'obj_home',
				'selectedtext' => __( 'Blog Homepage', 'seo-ultimate-pro' )
			);
		}
		...
		if ( ! $include || in_array( 'author', $include ) ) {
			...
			if ( count( $authors ) ) {
				...
				$items[] = array( 'text' => __( 'Author Archives', 'seo-ultimate-pro' ), 'isheader' => true );
				...
				foreach ( $authors as $author ) {
					...
					,
						'selectedtext' => $author->user_login . '<span class="type"> &mdash; ' . __( 'Author', 'seo-ultimate-pro' ) . '</span>'
					);
				}
			}
		}
				...
				foreach ( $aliases as $alias_id => $alias ) {
					...
					if ( $alias['to'] ) {
						...
						if ( ( strpos( $alias['from'], $_GET['q'] ) !== false ) || ( strpos( $to_rel_url, $_GET['q'] ) !== false ) ) {
							...
							if ( ! $header_outputted ) {
								$items[]          = array(
									'text'     => __( 'Link Masks', 'seo-ultimate-pro' ),
									'isheader' => true
								);
							...
							,
								'selectedtext' => $to_rel_url . '<span class="type"> &mdash; ' . __( 'Link Mask', 'seo-ultimate-pro' ) . '</span>'
							);
	...
	function su_add_dashboard_widgets() {
		...
		wp_add_dashboard_widget( 'sdf_dashboard_widget', 'From the Creators of SEO Ultimate Pro', array(
			&$this,
			'sdf_dashboard_widget_function'
		) );
